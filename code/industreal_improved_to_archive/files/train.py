    ...
    for step, (images, targets) in enumerate(pbar):
        ...
        if is_seq_batch:
            ...
            with amp.autocast('cuda', enabled=C.MIXED_PRECISION):
                ...
                # Save original flags before PSR-only override
            ...
            if not torch.isfinite(loss_seq):
                ...
                torch.cuda.empty_cache()
                # Restore criterion flags after PSR-only override
...
def main(args):
    ...
    # Match backbone params for both ResNet ('layer0'-'layer4') and ConvNeXt ('backbone.')
    _backbone_prefixes = ['layer0', 'layer1', 'layer2', 'layer3', 'layer4', 'backbone.']
    for name, param in model.named_parameters():
        if not param.requires_grad:
            continue
        if any(ln in name for ln in _backbone_prefixes):
