...
class MultiTaskLoss(nn.Module):
    ...
    def forward(self, outputs: Dict, targets: Dict) -> Tuple[torch.Tensor, Dict]:
        ...
        if self.use_kendall:
            ...
            # Combined pose loss: body keypoints (Wing Loss) + head pose (9-DoF MSE)
            # share a single Kendall weight prec_hp per paper §Multi-Task Loss.
            # BUG FIX: previously used elif, which dropped loss_head_pose when
            # train_pose=True. Both must be added under prec_hp.
            pose_combined = zero.clone()
            if self.train_pose:
                pose_combined = pose_combined + loss_pose.clamp(min=0.0)
            loss_head_pose = loss_head_pose.clamp(min=0.0)
            pose_combined = pose_combined + loss_head_pose
            if self.train_pose or loss_head_pose.item() > 0:
                total = total + prec_hp * pose_combined + lv_hp
            if self.train_act:
            ...
            if not math.isfinite(total_val):
                ...
                parts.append(loss_head_pose)  # always include head_pose
                if self.train_act:
            ...
            # Combine both pose-related losses (body keypoints + head pose 9-DoF)
            _loss_pose_staged = (loss_pose if self.train_pose else zero) + loss_head_pose
            if bool(getattr(C, 'STAGED_TRAINING', True)) and self._current_epoch >= 1:
                stage = _get_kendall_stage(self._current_epoch)
                if stage == 1:
                    ...
                    _loss_act_staged = zero
                    _loss_psr_staged = zero
                    # Stage 2: detection + head_pose active (do NOT zero pose)
