...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--metrics', nargs='+',
                        default=['det_mAP50', 'act_accuracy', 'act_macro_f1',
                                 'head_pose_MAE', 'psr_macro_f1', 'fps'],
                        help='Metrics to include in the table')
    parser.add_argument('--output', type=str, default='ablation_table.md',
                        help='Output markdown file')
    parser.add_argument('--sort-by', type=str, default=None,
                        help='Metric to sort rows by (default: first metric)')
    parser.add_argument('--higher-is-better', type=str, default='det_mAP50,act_accuracy,act_macro_f1,psr_macro_f1',
...
def render_table(
    ...
    lines = []
    lines.append('| Configuration | ' + ' | '.join(METRIC_DISPLAY_NAMES.get(m, m) for m in metric_names) + ' |')
    ...
    if sort_idx is not None:
        runs = sorted(
            runs,
            key=lambda r: (
                extract_metric(r['data'], metric_names[sort_idx]) or float('-inf')
            ),
