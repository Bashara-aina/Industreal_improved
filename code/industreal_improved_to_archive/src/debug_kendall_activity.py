...
def collate_fn(batch):
    ...
    return ({'rgb': images, 'clip_rgb': clip_rgb}, {
        'detection': [{'boxes': b['gt_boxes']['rgb'], 'labels': b['gt_classes']['rgb']} for b in batch],
        'head_pose': head_pose, 'psr_labels': psr_labels,
        'activity': action_label, 'hand_joints': hand_joints, 'box_mask': None,
    })


def get_ah_grads(model):
    return {n: p.grad.norm().item() for n, p in model.named_parameters()
...
print(f"  prec_act = exp(-log_var_act) = {torch.exp(-criterion.log_var_act.clamp(-4,2)).item():.6f}")
...
class TraceHook:
    ...
    def __call__(self, module, grad_input, grad_output):
        self.count += 1
        if self.count <= 3:
            print(f"  [Hook] {self.name}: grad_out={grad_output[0].norm().item() if grad_output[0] is not None else None}")
...
print(f"  act_logits4.grad: {act_logits4.grad.norm().item() if act_logits4.grad is not None else None}")
