    ...
    def _parse_ar_labels(self) -> np.ndarray:
        ...
        for start, end, action_id in spans:
            ...
            if start < self._num_frames:
                shifted = action_id + 1 if action_id > 0 else 0
...
class IndustRealMultiTaskDataset(Dataset):
    ...
    def __init__(
        ...
        # Per-recording sorted frame paths for VideoMAE clip loading (Doc 02 A.1)
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        # VideoMAE clip: [T, 3, 224, 224] normalized (Doc 02 A.1)
    ...
    def _load_clip_frames(
        ...
        Doc 02 D.3: Random temporal stride — sample T frames at a random stride
        ...
        # Random temporal stride: sample every N frames (Doc 02 D.3)
        if self.augment:
    ...
    def _scan_and_index(self) -> List[Dict[str, Any]]:
        ...
        for rec_dir in sorted(recordings_root.iterdir()):
            ...
            # Store sorted frame list for clip loading (Doc 02 A.1)
...
def collate_fn(batch: List[Dict[str, Any]]) -> Tuple[torch.Tensor, Dict[str, Any]]:
    ...
    # VideoMAE clip_rgb: [B, T, 3, 224, 224] — only present when USE_VIDEOMAE=True (Doc 02 A.1)
