...
def compute_pose_metrics(
    ...
    dof_names = ['forward_x', 'forward_y', 'forward_z',
                 'pos_x', 'pos_y', 'pos_z',
                 'up_x', 'up_y', 'up_z']
    result = {}
...
def main():
    ...
    logger.info(f'Found recording dirs for {len(rec_dir_map)}/{len(rec_ids_in_batch)} unique recordings')
    ...
    print(f'  {"Forward mean norm":20s} {fwd_norms_raw.mean():>14.6f}  {fwd_norms_fixed.mean():>14.6f}')
    print(f'  {"Forward std norm":20s} {fwd_norms_raw.std():>14.6f}  {fwd_norms_fixed.std():>14.6f}')
    print(f'  {"Forward min norm":20s} {fwd_norms_raw.min():>14.6f}  {fwd_norms_fixed.min():>14.6f}')
    print(f'  {"Forward max norm":20s} {fwd_norms_raw.max():>14.6f}  {fwd_norms_fixed.max():>14.6f}')
    print(f'  {"Forward drift >1%":20s} {(np.abs(fwd_norms_raw - 1.0) > 0.01).mean()*100:>14.1f}%  {"0.0%":>15s}')
    print(f'  {"Forward drift >5%":20s} {(np.abs(fwd_norms_raw - 1.0) > 0.05).mean()*100:>14.1f}%  {"0.0%":>15s}')
    print(f'  {"Forward drift >10%":20s} {(np.abs(fwd_norms_raw - 1.0) > 0.10).mean()*100:>14.1f}%  {"0.0%":>15s}')
    ...
    print(f'  {"Up drift >1%":20s} {(np.abs(up_norms_raw - 1.0) > 0.01).mean()*100:>14.1f}%  {"0.0%":>15s}')
    print(f'  {"Up drift >5%":20s} {(np.abs(up_norms_raw - 1.0) > 0.05).mean()*100:>14.1f}%  {"0.0%":>15s}')
    ...
    fwd_ang_delta = metrics_fixed.get('forward_angular_MAE_deg', 0) - metrics_raw.get('forward_angular_MAE_deg', 0)
    up_ang_delta = metrics_fixed.get('up_angular_MAE_deg', 0) - metrics_raw.get('up_angular_MAE_deg', 0)
    ...
    raw_fwd_delta = sum(abs(metrics_fixed.get(k, 0) - metrics_raw.get(k, 0)) for k in fwd_component_keys)
    raw_up_delta = sum(abs(metrics_fixed.get(k, 0) - metrics_raw.get(k, 0)) for k in up_component_keys)
