...
def run_smoke_test():
    ...
    print(f"\n[TRAIN] {num_epochs} epochs, {steps_per_epoch} steps/epoch, "
          f"batch={batch_size}")
    print(f"  {'=' * 50}")
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if epoch <= 20 or epoch % 20 == 0 or epoch == num_epochs:
            ...
            print(f"  ep {epoch:4d}/{num_epochs} lr {lr:.2e}"
        ...
        if (avg['det'] < 0.8 and avg['act'] < 0.8 and avg['psr'] < 0.8
                and avg['pose'] < 0.08):
    ...
    # ── Save report ──
    output_dir = Path(_WORK_DIR) / 'src' / 'runs' / 'rf_stages' / 'checkpoints' / 'smoke_test_4heads'
