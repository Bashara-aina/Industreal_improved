...
def load_checkpoint_compat(path: str, model, device: torch.device):
    ...
    model_state = ckpt.get("model_state_dict", ckpt.get("model_state", ckpt.get("model", ckpt)))
...
def main():
    ...
    parser.add_argument(
        "--max-batches", type=int, default=0, help="0 = full eval, else limit batches"
    )
    parser.add_argument(
        "--save-dir", type=str, default=None, help="Output directory (default: checkpoint dir)"
    )
    ...
    loader = DataLoader(
        ds,
        batch_size=int(getattr(C, "VAL_BATCH_SIZE", C.BATCH_SIZE)),
        shuffle=False,
        num_workers=int(getattr(C, "VAL_NUM_WORKERS", 4)),
    ...
    results = evaluate_all(
        model, criterion, loader, device, max_batches=max_batches, save_dir=save_dir
    )
