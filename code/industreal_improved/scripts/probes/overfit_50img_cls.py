...
def build_model(device):
    ...
    for name, param in model.named_parameters():
        if not any(
            det_key in name for det_key in ["det_head", "detection_head", "fpn", "backbone"]
        ):
...
def run_overfit(args):
    ...
    print(
        f"  DET_OHEM_ENABLED={C.DET_OHEM_ENABLED}  KENDALL_FIXED_WEIGHTS={C.KENDALL_FIXED_WEIGHTS}"
    )
    ...
    print(
        f"  Using {len(train_indices)} training images (indices: {train_indices[:5]}...), "
    ...
    print(
        f"  Det params: {sum(p.numel() for p in det_params) / 1e3:.1f}K, "
    ...
    print(
        f"  {'Epoch':>5} | {'Loss':>8} | {'ClsL':>8} | {'RegL':>8} | "
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch == 1 or epoch % args.log_every == 0 or epoch == args.epochs:
            print(
                f"  {epoch:>5d} | {epoch_loss:>8.4f} | {epoch_cls:>8.4f} | {epoch_reg:>8.4f} | "
