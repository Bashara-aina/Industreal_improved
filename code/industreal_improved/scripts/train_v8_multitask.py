    ...
    def __getitem__(self, idx):
        ...
        if clip is None:
            ...
            return torch.zeros(3, self.num_frames, 224, 224), {
                'activity': 0, 'pose': torch.zeros(6), 'psr': torch.zeros(self.num_psr_comps),
                'recording': s['recording'], 'start_frame': s['start_frame'],
            }
...
class V8Model(nn.Module):
    ...
    def forward(self, clip):
        ...
        return {
            ...
            'psr_logits': torch.stack([h(feat) for h in self.psr_head], dim=1).squeeze(-1),  # [B, 11]
        }

    def compute_loss(self, out, targets):
        losses = {}
        # Activity (cross-entropy)
        activity_targets = torch.tensor([t["activity"] for t in targets], dtype=torch.long).to(out["activity_logits"].device)
...
def main():
    ...
    parser.add_argument('--save-dir', default='/media/newadmin/master/POPW/working/code/industreal_improved/code/industreal_improved/src/runs/v8_multitask')
    parser.add_argument('--val-csv', default='/media/newadmin/master/POPW/datasets/industreal/recordings/val')  # can be split from train.csv
    ...
    val_csv = Path('/media/newadmin/master/POPW/datasets/industreal/recordings/val') / 'val_split.csv'
    if not val_csv.exists():
        ...
        val_loader = torch.utils.data.DataLoader(val_ds_subset, batch_size=args.batch_size, shuffle=False,
                                                  num_workers=args.num_workers, collate_fn=collate_fn)
    else:
        ...
        val_loader = torch.utils.data.DataLoader(val_ds, batch_size=args.batch_size, shuffle=False,
                                                  num_workers=args.num_workers, collate_fn=collate_fn)
    logger.info(f"Train: {len(train_ds)} samples, Val: {len(val_ds) if val_csv.exists() else len(val_ds_subset)} samples")
    ...
    for epoch in range(args.epochs):
        ...
        for i, (clip, targets) in enumerate(train_loader):
            ...
            if i % 50 == 0:
                logger.info(f"Epoch {epoch} step {i} | act={losses['act'].item():.3f} pose={losses['pose'].item():.3f} psr={losses['psr'].item():.3f}")
        elapsed = time.time() - t0
        logger.info(f"Epoch {epoch} done in {elapsed:.1f}s ({train_metrics['count']} samples)")
        if train_metrics['count'] > 0:
            logger.info(f"  Train: act={train_metrics['act_loss']/train_metrics['count']:.3f} pose={train_metrics['pose_loss']/train_metrics['count']:.3f} psr={train_metrics['psr_loss']/train_metrics['count']:.3f}")

        # Val
        val_metrics = val_step(model, val_loader, device)
        if val_metrics:
            logger.info(f"  Val:   act={val_metrics['act_loss']:.3f} pose={val_metrics['pose_loss']:.3f} psr={val_metrics['psr_loss']:.3f}")
