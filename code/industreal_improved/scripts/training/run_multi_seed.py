...
# train.py and evaluate.py live in src/training/ and src/evaluation/
...
def run_evaluation(
    ...
    eval_cmd = [
        sys.executable,
        str(SRC_DIR / "evaluation" / "evaluate.py"),
        "--checkpoint",
        str(checkpoint),
        "--split",
        "val",
...
def print_summary_table(all_results: List[Dict[str, Any]]) -> None:
    ...
    headline_metrics = [
        ...
        ("psr_f1_at_t", "PSR F1@±3frames"),
        ("psr_edit_score", "PSR Edit Score"),
        ("head_pose_MAE", "Head Pose MAE (deg)"),
        ("head_pose_angular_MAE_deg", "Head Pose Angular MAE"),
    ...
    print("=" * 80)
    header = (
        f"  {'Metric':<28} " + "".join(f"  Seed {r['seed']:<8}" for r in all_results) + "  Mean±Std"
    )
    print(header)
    print("  " + "-" * 76)
