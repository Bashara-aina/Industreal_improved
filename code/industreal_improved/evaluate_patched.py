...
def evaluate_all(
    ...
    try:
        ...
        # DEBUG: Check detection_list before loop
        _debug_det_list_len = len(detection_list)
        logger.info(f"  DEBUG: detection_list has {_debug_det_list_len} items")
        ...
        for i in range(B):
            ...
            # DEBUG: Check detection_list[-1] before append
            _debug_boxes = detection_list[i]["boxes"].cpu().numpy()
            logger.info(f"  DEBUG Batch{bi} Sample{i}: boxes shape={_debug_boxes.shape}, labels shape={detection_list[i]["labels"].cpu().numpy().shape}")
