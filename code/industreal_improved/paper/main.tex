...
\begin{abstract}
We present a multi-task system sharing a single ConvNeXt-Tiny backbone across four industrial assembly perception tasks on the IndustReal dataset: object detection (24-class ASD), activity recognition (75-way clip classification), procedure step recognition (11 binary PSR components), and head pose estimation (9-DoF). At 46.47M total parameters (ConvNeXt-Tiny backbone, FPN, four task heads, two FiLM modules), the model operates within consumer GPU memory (RTX 3060/5060~Ti). We characterize three multi-task learning pathologies: (1) PSR dead-ReLU collapse resolved by transition-target supervision (F1 0\,$\to$\,0.7018), (2) Kendall uncertainty-weighting log-var collapse mitigated by per-task clamping bounds including KENDALL\_HP\_PREC\_CAP (pose precision $\leq$ detection precision), and (3) a 20,245$\times$ gradient-norm imbalance between pose and PSR heads traced to tensor-dimensionality artifacts. The activity label distribution is severely long-tailed (75 classes; 16 with $<$10 frames). Per-task multi-task versus single-task accounting under a fixed budget across three seeds with bootstrap confidence intervals yields \resultplaceholder{activity top-1}, \resultplaceholder{detection mAP50-pc}, \resultplaceholder{PSR F1@$\pm$3}, and \resultplaceholder{head pose MAE}. We establish the first head-pose baseline on IndustReal with MediaPipe comparison.
\end{abstract}
...
\textbf{Body pose head (1.64M).} Predicts 17 COCO keypoints using pseudo-labels derived from MediaPipe hand landmarks (see~\cref{sec:limitations}). This head was included for completeness and is not part of our primary experimental comparison.
...
Training proceeds through ten progressive stages (RF1--RF10)\footnote{Earlier versions of this document described a three-stage curriculum. The live codebase has always implemented the ten-stage schedule described here; the V1 claim was an error in paper drafting.} orchestrated by a stage manager that unlocks heads, scales the training subset, and tightens per-task metric gates. RF1 trains detection only (20\% data, 20 max epochs, gate: det mAP50 $\ge$ 0.15). RF2 adds both pose heads (50\% data, 30 ep, gate: det 0.22 / pose MAE $\le$ 70). RF3 adds activity (35\% data, 15 ep, gate: det 0.20 / act top1 $\ge$ 0.05 / pose 70). RF4 adds PSR---all four heads active from this point (50\% data, 20 ep, gate: det 0.20 / act 0.06 / PSR F1 $\ge$ 0.05 / pose 65). RF5--RF9 progressively scale data from 50\% to 90\% (10 ep each) with tightening gates culminating in RF10 (100\% data, 15 ep, gate: det 0.30 / act 0.18 / PSR 0.16 / pose 35). The non-monotonic data drop at RF3 (50\%~$\to$~35\%) compensates for adding the activity head.

The LR schedule uses OneCycleLR with a two-epoch linear warmup (LinearLR, start\_factor=1e-5); cosine annealing is not used. The peak LR follows the Goyal linear scaling rule---effective batch 48 yields a 1.5$\times$ peak factor. Stochastic Weight Averaging (SWA) is disabled; only exponential moving average (EMA) is active. The config parameter STAGED\_TRAINING (legacy scheme with STAGE1\_EPOCHS=5, STAGE2\_EPOCHS=10, STAGE3\_EPOCHS=85) is inactive---the stage manager above is the active curriculum.

Training hyper-parameters: effective batch size 48 (batch 6 $\times$ gradient accumulation 8); bf16 precision only (FP16 is disallowed---PSR BCE overflows FP16); gradient clipping at 5.0; optimiser AdamW.
...
For context, WACV~2024~\cite{schoonbeek2024industreal} published single-task baselines on \industreal{} using different protocols: MViTv2-S for activity (65.25\% top-1, RGB+optical flow), YOLOv8m for detection (0.838~mAP50 at 1280px), and EfficientNet-B3 for PSR (0.883~F1, clip-level transition paradigm). These are cited as contextual references only---differences in input resolution (1280px vs.\ 224px, a 32.7$\times$ pixel-count gap for detection), input modality (RGB+flow vs.\ RGB-only for activity), and learning paradigm (single-task vs.\ multi-task) preclude direct comparison. We make no head-to-head claim against any published work. STORM-PSR~\cite{schoonbeek2025storm} (0.506~F1) provides a spatio-temporal tube reference on the same dataset under yet another paradigm. Comparisons with these references are informative for calibration, not for ranking.
...
\begin{itemize}
    ...
    \item \textbf{Body pose provenance.} The body pose head predicts 17 COCO-style keypoints, but IndustReal contains no ground-truth body keypoint annotations. Pseudo-labels are derived from MediaPipe left-hand landmarks (26 joints, down-sampled to 17, normalized to $[0,1]$ pixel coordinates) rather than from detection bounding boxes as previously stated. This head is trained with Wing Loss ($\omega=0.05$, $\varepsilon=0.005$) at weight 5.0, contributing to the Kendall total alongside body pose log-variance regularization. The pseudo-label quality and distribution mismatch (hand vs.\ full-body keypoints) limit the head's independent value; we include it for architectural completeness and leave genuine body pose annotation to future work.
