...
def _compute_smoke_losses(model, model_name, batch, device):
    ...
    if model_name == 'TierFModel':
        ...
        loss_psr = F.binary_cross_entropy_with_logits(
            temp_out['psr_logits'], targets['psr_labels']
        )
...
def test_all_four_losses_produced(model_4head, sample_batch, device):
    ...
    for head in expected_heads:
        ...
        assert torch.isfinite(losses[head]), \
            f"Loss for {head} is not finite: {losses[head].item()}"

    print(f"  Losses: det={losses['det'].item():.4f} act={losses['act'].item():.4f} "
    ...
    for name, p in model.named_parameters():
        if p.grad is not None:
            ...
            if grad_norm > 0 and not has_nan and not has_inf:
                ...
                for head in ['backbone', 'fpn', 'detection', 'activity', 'psr', 'pose', 'head_pose']:
                    if head in name:
