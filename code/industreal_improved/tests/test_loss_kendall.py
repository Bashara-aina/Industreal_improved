...
sys.path.insert(
    0, os.path.join(work_dir, "src")
)  # src/ at index 0 → 'import training' finds src/training/
sys.path.insert(1, work_dir)  # project root at index 1


LOSSES = os.path.join(work_dir, "src/training/losses.py")
...
class TestKendallFormula:
    ...
    def test_kendall_precision_computation(self):
        ...
        assert "prec_det = torch.exp(-lv_det)" in src, "det precision = exp(-lv_det)"
        assert "prec_hp = torch.exp(-lv_hp)" in src, "pose precision = exp(-lv_hp)"
        assert "prec_act = torch.exp(-lv_act)" in src, "act precision = exp(-lv_act)"
        assert "prec_psr = torch.exp(-lv_psr)" in src, "psr precision = exp(-lv_psr)"
        # Clamp first, then exp
        assert "lv_det = self.log_var_det.clamp(-4.0, 2.0)" in src, "log_var_det clamped to [-4, 2]"
    ...
    def test_kendall_clamp_range(self):
        ...
        src = open(LOSSES).read()
        assert ".clamp(-4.0, 2.0)" in src, "log_var clamp should be (-4.0, 2.0) per paper spec"

    def test_activity_ramp_min_one(self):
        """Verify act_ramp = min(1, epoch/5) does not exceed 1."""
        src = open(LOSSES).read()
        ramp_pattern = r"act_ramp\s*=\s*min\(1"
        assert re.search(ramp_pattern, src), "act_ramp = min(1, ...) not found"
...
class TestStagedPrecisionZeroing:
    ...
    def test_stage1_kendall_zeros_act_psr_pose(self):
        ...
        src = open(LOSSES).read()
        stage1 = re.search(
            r"if stage == 1:(.*?)(?=elif stage == 2|else:|total = torch)", src, re.DOTALL
        )
    ...
    def test_stage2_kendall_zeros_act_psr_not_pose(self):
        ...
        src = open(LOSSES).read()
        stage2 = re.search(
            r"elif stage == 2:(.*?)(?=elif stage == 3|else:|total = torch)", src, re.DOTALL
        )
