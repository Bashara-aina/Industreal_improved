...
class TestF4OneCyclePeak:
    ...
    def test_torch_restores_max_lr_on_load(self):
        ...
        torch.optim.lr_scheduler.OneCycleLR(opt, max_lr=[1e-3], total_steps=100, pct_start=0.1)
        state = opt.state_dict()
        opt2 = torch.optim.AdamW([{"params": [p], "lr": 1e-3}])
        torch.optim.lr_scheduler.OneCycleLR(opt2, max_lr=[2e-3], total_steps=100, pct_start=0.1)
...
class TestF14LogVarWeightDecay:
    def test_loss_param_group_has_zero_wd(self):
        src = open(TRAIN_PY).read()
        assert src.count("{'params': loss_params, 'lr': head_lr, 'weight_decay': 0.0}") == 2, (
...
class TestF16AblationPresets:
    @pytest.mark.parametrize(
        ...
        [
            (
                "ablation_det_only",
                dict(TRAIN_DET=True, TRAIN_ACT=False, TRAIN_PSR=False, TRAIN_HEAD_POSE=False),
            ),
            (
                "ablation_act_only",
                dict(TRAIN_DET=False, TRAIN_ACT=True, TRAIN_PSR=False, TRAIN_HEAD_POSE=False),
            ),
            (
                "ablation_psr_only",
                dict(TRAIN_DET=False, TRAIN_ACT=False, TRAIN_PSR=True, TRAIN_HEAD_POSE=False),
            ),
            (
                "ablation_pose_only",
                dict(TRAIN_DET=False, TRAIN_ACT=False, TRAIN_PSR=False, TRAIN_HEAD_POSE=True),
            ),
        ],
    )
    ...
    def test_act_only_zeroes_det_conf(self):
        ...
        assert C.ZERO_DET_CONF_FOR_RECOVERY is True, (
            "act-only ablation must not feed untrained det confidences into the activity input"
        )
...
class TestF18SingleActivityRamp:
    def _total_at(self, counter, ramp_epochs=4):
        ...
        crit = MultiTaskLoss(train_det=False, train_pose=False, train_act=True, train_psr=False)
...
class TestF21AutoPeakFactor:
    ...
    def test_config_default_is_auto(self):
        ...
        assert (
            str(C.ONE_CYCLE_PEAK_FACTOR).lower() in ("auto",) or float(C.ONE_CYCLE_PEAK_FACTOR) > 0
        ), "peak factor misconfigured"
