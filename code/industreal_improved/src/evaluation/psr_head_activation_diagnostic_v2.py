...
def print_stats(stats: dict):
    ...
    print(
        f"  {stats['name']:40s}  mean={stats['mean']:10.6f}  "
...
def analyze_batch(hook_state, batch_idx: int):
    ...
    for i in range(nc):
        ...
        if pre_gelu is not None:
            ...
            print(
                f"  {'':40s}  pre-GELU mean={pre_stats['mean']:10.6f} std={pre_stats['std']:10.6f}"
            )

        if logits is not None:
            logit_stats = compute_stats(logits, f"comp[{i:02d}] logits   [{list(logits.shape)}]")
            print(
                f"  {'':40s}  logits  mean={logit_stats['mean']:10.6f} std={logit_stats['std']:10.6f}"
            )
...
def main():
    ...
    parser.add_argument(
        "--num-batches", type=int, default=DIAG_BATCHES, help="Number of batches to forward"
    )
    parser.add_argument(
        "--sequence-length", type=int, default=8, help="Sequence length for PSR temporal context"
    )
    parser.add_argument(
        "--batch-size", type=int, default=2, help="Batch size (number of sequences)"
    )
    ...
    print(f"\nTransformer output:")
    print(
        f"  encoded.std() — mean: {np.mean(encoded_stds):.6f}, "
    ...
    for i in range(NUM_COMPONENTS):
        ...
        if post_gelu_all:
            ...
            print(
                f"  comp[{i:02d}]: mean={mean_val:10.6f} std={std_val:10.6f} "
    ...
    if len(all_dead) > int(NUM_COMPONENTS * 0.5):
        ...
        print(
            f"     fix transformer variance (increase d_model dropout or use LayerNorm after transformer)."
        )
    elif len(all_dead) > 0:
        print(f"  -> Some GELU heads saturated, some alive. Mixed picture.")
        print(
            f"     Repair: LeakyReLU(0.01) + bias=0.0 on output heads, or individual variance scaling."
        )
    else:
