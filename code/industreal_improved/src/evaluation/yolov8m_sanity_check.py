    ...
    for rec_dir in sorted(VAL_DIR.iterdir()):
        ...
        for f in sorted(rgb_dir.iterdir()):
            if f.suffix.lower() in ('.jpg', '.jpeg', '.png'):
                images.append(f)
    logger.info('Found %d val images across %d recordings', len(images), len(list(VAL_DIR.iterdir())))
    return images
...
def run_sanity_check(
    ...
    if num_samples > len(images):
        logger.warning(
            ...
            num_samples, len(images), len(images),
        )
    ...
    for img_path in sampled:
        ...
        logger.info(
            ...
            rel_path, len(pred_classes),
            pred_classes[:5], [f'{s:.3f}' for s in pred_scores[:5]],
        )
...
def main():
    ...
    parser.add_argument(
        '--out_path', default='sanity_check.json',
