...
def _val_worker(
    ...
    val_batch_size = int(
        overrides.get("VAL_BATCH_SIZE", getattr(C, "VAL_BATCH_SIZE", C.BATCH_SIZE))
    )
    _ds_module = sys.modules.get("src.data.industreal_dataset") or __import__(
        "src.data.industreal_dataset", fromlist=["collate_fn"]
    )
    ...
    logger.info(
        "[SUB] Starting evaluate_all (max_batches=%s, batch_size=%d) ...",
...
def run_val_subprocess(
    ...
    p = _CTX.Process(
        ...
        args=(
            ...
            str(out_path),
            overrides,
            str(predictions_path) if predictions_path else None,
        ),
    )
    p.start()
    ...
    while _elapsed < timeout:
        ...
        if _elapsed >= timeout // 2 and _log_timeout_warning:
            logger.warning(
                "[SUB] Validation subprocess still running after %d s (PID=%d, will kill at %d s)",
                _elapsed,
                p.pid,
                timeout,
            )
