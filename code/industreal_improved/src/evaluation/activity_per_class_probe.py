...
def main():
    try:
        ...
        backbone_state = {
            k[len("backbone.") :]: v for k, v in model_state.items() if k.startswith("backbone.")
        }
        ...
        logger.info(
            f"Backbone loaded ({sum(p.numel() for p in backbone.parameters()):,} params, frozen)"
        )
        ...
        logger.info(
            f"Train features: {train_feats.shape} ({train_skip} skipped) in {time.time() - t0:.0f}s"
        )
        ...
        logger.info(
            f"Val features: {val_feats.shape} ({val_skip} skipped) in {time.time() - t0:.0f}s"
        )
        ...
        logger.info(
            f"Per-recording majority baseline: {per_rec_baseline:.4f} ({rec_correct}/{rec_total})"
        )
        ...
        logger.info(
            f"Global majority baseline: {global_majority_baseline:.4f} (class {global_majority_class})"
        )
        ...
        per_rec_accuracy = {
            rec: round(d["correct"] / d["total"], 4) for rec, d in rec_pred_groups.items()
        }
        ...
        logger.info(
            f"Classes with signal (acc > 1/{num_classes}): {classes_with_signal}/{num_classes}"
        )
        logger.info(f"Classes with zero accuracy: {classes_with_zero}/{num_classes}")
        logger.info(
            f"Mean per-class acc: {np.mean([v['accuracy'] for v in per_class_acc.values()]):.4f}"
        )
        ...
        results = {
            ...
            "mean_per_class_accuracy": round(
                float(np.mean([v["accuracy"] for v in per_class_acc.values()])), 4
            ),
