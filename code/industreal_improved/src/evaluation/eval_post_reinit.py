...
OUT_DIR = Path(
    f"/home/newadmin/swarm-bot/project/popw/working/code/industreal_improved/src/runs/{RUN_NAME}"
)
...
# PATCH P6 [opus RC-17]: Always use standard collate for eval. The sequence-mode
# collate_fn_sequences drops clip_rgb from the batch, which causes half the
# activity classifier input (the VideoMAE stream) to be zero-filled during eval
# (model.py:1347-1348 zeros_like fallback). Eval is never in sequence mode, and
# the misalignment inflated the EMA-vs-standalone-eval metric gap.
...
print(
    f"[reinit-eval] load: missing={len(res.missing_keys)} unexpected={len(res.unexpected_keys)}",
    flush=True,
)

# Verify no NaN/Inf in loaded params
nan_params = [
    n for n, p in model.named_parameters() if torch.isnan(p).any() or torch.isinf(p).any()
]
...
if SKIP_REINIT:
    print(
        "[reinit-eval] EVAL_SKIP_REINIT=1: NOT re-initializing heads (using trained post-retrain weights)",
        flush=True,
    )
    init_counts = {}
else:
...
# PATCH P8 [opus OQ-3]: The default MultiTaskLoss() uses num_classes_act=74
# and num_psr_components=11, but the model was trained with
# C.NUM_CLASSES_ACT=75 and C.NUM_PSR_COMPONENTS=11. For activity this is
# benign (_fit_to_width in LDAM adjusts to actual logits width), but pass
# the correct values for hygiene.
...
def _sanitize(o):
    if isinstance(o, dict):
        return {
            k: _sanitize(v) for k, v in o.items() if not isinstance(v, (torch.Tensor, np.ndarray))
        }
...
try:
    ...
    print(
        f"[reinit-eval] _print_single_run_results missing key {e} (likely no efficiency profile)",
        flush=True,
    )
