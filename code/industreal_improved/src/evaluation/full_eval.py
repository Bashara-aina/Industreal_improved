...
def main():
    ...
    if psr_logits_list:
        ...
        for thresh in np.arange(0.1, 1.0, 0.05):
            ...
            if f1 > best_f1:
                ...
                best_t = thresh
        comp_acc = (
            ((sig > best_t).astype(np.int32)[valid] == all_labels[valid]).mean()
            if valid.sum() > 0
            else 0
        )
        results["psr"] = {
        ...
        print(
            f"  sigmoid range: [{sig[valid].min():.3f}, {sig[valid].max():.3f}], mean={sig[valid].mean():.3f}"
        )
    ...
    if act_logits_list:
        ...
        valid = act_labels >= 0
        top1_valid = (
            (act_preds[valid] == act_labels[valid]).float().mean().item() if valid.sum() > 0 else 0
        )
        ...
        try:
            macro_f1 = f1_score(
                act_labels.numpy(), act_preds.numpy(), average="macro", zero_division=0
            )
        except:
            macro_f1 = 0
        results["activity"] = {
