...
def extract_features_and_metadata(
    ...
    for batch_idx, batch in enumerate(loader):
        if batch_idx >= effective_max:
            logger.info(
                f"  {desc}: stopping at batch {batch_idx}/{effective_max} (max_batches)"
            )
            break
        ...
        if batch_idx % 50 == 0:
            ...
            if batch_idx == 0:
                ...
                import sys; sys.stdout.flush()  # ensure unbuffered
            else:
                logger.info(
...
def form_clips(
    ...
    for i, rec_id in enumerate(rec_ids):
        rec_groups[rec_id].append(
            (frame_nums[i], features[i].clone(), labels[i].item())
        )
...
def main():
    parser = argparse.ArgumentParser(
        description='Activity temporal probe (CPU, OOM-safe)'
    )
    parser.add_argument(
        '--save-dir', type=str,
    ...
    train_dataset = IndustRealMultiTaskDataset(
        split='train', augment=False, subset_ratio=1.0,
    )
    val_dataset = IndustRealMultiTaskDataset(
        split='val', augment=False, subset_ratio=1.0,
    )
    ...
    train_data = extract_features_and_metadata(
        backbone, train_loader, desc="Train",
    ...
    train_clip_feat, train_clip_lbl, train_rec_counts = form_clips(
        train_data['rec_ids'], train_data['frame_nums'],
    ...
    val_clip_feat, val_clip_lbl, val_rec_counts = form_clips(
        val_data['rec_ids'], val_data['frame_nums'],
        val_data['features'], val_data['labels'],
        clip_size=args.clip_size, stride=args.stride,
    )

    logger.info(
        f'Train clips: {train_clip_feat.shape[0]}, '
        f'Val clips: {val_clip_feat.shape[0]}'
    )
    logger.info(
        f'Val clips/recording: {dict(sorted(val_rec_counts.items()))}'
    )
    ...
    clip_majority_baseline, clip_majority_class = compute_majority_baseline(
        val_clip_lbl.numpy()
    )
    logger.info(
        f'Clip-level majority baseline: {clip_majority_baseline:.4f} '
        f'(class {clip_majority_class})'
    )
    ...
    per_class_metrics = compute_per_class_metrics(
        best_val_labels, best_val_preds, num_classes,
    )
    f1_scores = [
        m['f1'] for m in per_class_metrics.values() if m['support'] > 0
    ]
    macro_f1 = np.mean(f1_scores) if f1_scores else 0.0
    weights = [
        m['support'] for m in per_class_metrics.values() if m['support'] > 0
    ]
    weighted_f1 = (
        np.average(f1_scores, weights=weights)
        if f1_scores and sum(weights) > 0 else 0.0
    )
    ...
    logger.info(
        f'Best clip mean-pool top-1 (epoch {best_epoch}): {best_val_top1:.4f}'
    )
    ...
    results = {
        ...
        'val_clips_per_recording': {
            k: int(v) for k, v in sorted(val_rec_counts.items())
        },
        'train_clips_per_recording': {
            k: int(v) for k, v in sorted(train_rec_counts.items())
        },
    ...
    zerof1 = sum(
        ...
        if m['support'] > 0 and m['f1'] == 0
    )
    md_lines.append(
        f'\nNon-zero classes: {nonzero}/{num_classes}\n'
    )
    md_lines.append(
        f'Non-predicted (zero F1 with support): {zerof1}\n'
    )
