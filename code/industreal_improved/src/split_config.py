...
_MANIFEST_PATH = Path(__file__).resolve().parent.parent / "config" / "splits" / "industreal_split.json"
...
def load_manifest() -> dict:
    ...
    if not _MANIFEST_PATH.exists():
        raise FileNotFoundError(
            f"Split manifest not found at {_MANIFEST_PATH}. "
            f"Expected to exist after setup."
        )
...
def require_split(eval_split: str, allow_test_only: bool = False) -> None:
    ...
    if eval_split not in ALL_SPLIT_IDS:
        valid = list(ALL_SPLIT_IDS.keys())
        raise ValueError(
            f"Invalid eval_split='{eval_split}'. Must be one of {valid}."
        )
