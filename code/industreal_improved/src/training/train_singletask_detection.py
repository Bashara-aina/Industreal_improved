#!/usr/bin/env python3
"""
Single-task detection-only ConvNeXt-Tiny training (Opus 140 Q4).

Patches src.config to enable ONLY the detection head (TRAIN_DET=True,
TRAIN_HEAD_POSE=False, TRAIN_ACT=False, TRAIN_PSR=False), then delegates
all training logic to train.py. All train.py CLI arguments are forwarded.

Purpose: produces a clean same-architecture detection-only denominator
for multi-task cost analysis.

Usage:
    python src/training/train_singletask_detection.py [train.py args...]

Examples:
    # Resume from crash_recovery checkpoint:
    python src/training/train_singletask_detection.py --batch-size 2 \\
...
# Enable bf16 mixed precision (~2x throughput). Original MIXED_PRECISION=False
# was set because PSR seq-loss spikes corrupted FP16 GradScaler — irrelevant
# for detection-only training. BF16 needs no GradScaler on RTX 5060 Ti.
...
# any code that references CFG_TRAIN_DET etc. reads the correct values.
train.CFG_TRAIN_DET = True
train.CFG_TRAIN_HEAD_POSE = False
train.CFG_TRAIN_ACT = False
train.CFG_TRAIN_PSR = False

# ---- Phase 4: Parse CLI args (mirrors train.py __main__) ----
parser = argparse.ArgumentParser(
    description="Single-task ConvNeXt-Tiny detection training (Opus 140 Q4)",
