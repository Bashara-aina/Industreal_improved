#!/usr/bin/env python3
"""
Single-task PSR-only ConvNeXt-Tiny training (Opus 141, 2nd baseline).

Patches src.config to enable ONLY the PSR head (TRAIN_PSR=True,
TRAIN_DET=False, TRAIN_HEAD_POSE=False, TRAIN_ACT=False), applies the
ablation_psr_only preset, then delegates all training logic to train.py.
All train.py CLI arguments are forwarded.

Purpose: third single-task baseline — isolates PSR to determine whether
multi-task training helps or hurts PSR performance.

Current multi-task PSR:  0.7018 F1
V3 (with fix, in flight): target 0.78+ F1
Single-task PSR:         unknown (key comparison vs multi-task)

Usage:
    python src/training/train_singletask_psr.py [train.py args...]

Examples:
    # Fresh start from COCO-pretrained backbone:
    python src/training/train_singletask_psr.py --batch-size 2 --max-epochs 5

    # Every batch is a sequence batch (recommended for PSR-only):
...
# bf16 mixed precision is safe for PSR-only because:
...
# any code that references CFG_TRAIN_PSR etc. reads the correct values.
train.CFG_TRAIN_PSR = True
