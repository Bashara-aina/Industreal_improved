    ...
    def __init__(
        ...
        if split_csv and Path(split_csv).exists():
            ...
            recording_ids = sorted(
                [d.name for d in (self.recordings_root / split).iterdir() if d.is_dir()]
            )
        ...
        logger.info(
            f"[{split}] Detection dataset: {len(self.samples)} frames from {len(recording_ids)} recordings"
        )
    ...
    def __getitem__(self, idx):
        ...
        if self.augment:
            ...
            if random.random() < mosaic_prob:
                ...
                quad_samples = [sample] + [
                    self.samples[random.randint(0, len(self.samples) - 1)] for _ in range(3)
                ]
...
def main(args):
    ...
    for epoch in range(start_epoch, epochs):
        ...
        logger.info(
            f"Train loss: {train_metrics['loss']:.4f}  lr: {optimizer.param_groups[0]['lr']:.2e}"
        )
