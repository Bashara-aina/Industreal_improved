...
def assess_det_health(det_health_lines: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    if near_zero > 0.5:
        issues.append(
            f"near_zero={near_zero:.2%} — majority logits near zero, head may be collapsing"
        )
    elif near_zero > 0.2:
    ...
    if cls_std < 0.1:
        issues.append(
            f"cls_std={cls_std:.4f} — extremely low, all logits nearly equal (degenerate)"
        )
    elif cls_std < 0.5:
...
def select_retry_strategy(
    state: StageState, stage_cfg: Optional[Dict[str, Any]] = None
) -> Dict[str, Any]:
    ...
    if stage_cfg and state.retry_count >= 1:
        ...
        logger.info(
            f"  Stage-aware strategy for {stage_name}: "
...
def run_pre_flight_checks(stage_cfg: Dict[str, Any]) -> List[str]:
    ...
    if not torch.cuda.is_available():
        ...
        logger.info(
            f"GPU: {free_mem:.0f}MB free / {total_mem:.0f}MB total (stage={stage_cfg.get('name', '?')})"
        )
        if free_mem < 2000:
...
def validate_checkpoint(path: Path) -> Tuple[bool, str]:
    ...
    return (
        True,
        f"Valid (epoch={epoch}, {len(state_dict)} tensors, {path.stat().st_size / 1e6:.0f}MB)",
    )


def estimate_progress(
    stage_cfg: Dict[str, Any], state: StageState, snapshot: LogSnapshot
) -> Dict[str, Any]:
...
def _select_cross_stage_strategy(state: StageState, stage_cfg: Dict[str, Any]) -> Optional[str]:
    ...
    if candidates:
        ...
        logger.info(
            f'  [CROSS-STAGE] Using strategy "{candidates[0][1]}" from stage {candidates[0][0]}'
        )
...
def launch_training(
    ...
    if resume_from and resume_from.exists():
        try:
            ...
            logger.info(
                f"  Checkpoint epoch={ckpt.get('epoch', 0)} → "
                f"training for {stage_cfg['max_epochs']} more → total --max-epochs={max_epochs}"
            )
        except Exception as e:
            logger.warning(
                f"  Could not read checkpoint epoch ({e}), using raw max_epochs={max_epochs}"
            )
    ...
    env["_STAGE_NAME"] = stage_cfg[
        "name"
    ]  # e.g. 'rf1', 'rf2' -- used by train.py for per-stage ckpt subdirs
    ...
    if strategy["lr_mult"] != 1.0:
        logger.info(
            f"  [STRATEGY] LR mult={strategy['lr_mult']}× (effective LR={5e-4 * strategy['lr_mult']:.2e})"
        )
    if strategy["warmup_mult"] != 1.0:
        logger.info(
            f"  [STRATEGY] Warmup mult={strategy['warmup_mult']}× (effective warmup={5 * strategy['warmup_mult']:.0f} epochs)"
        )
...
def evaluate_gate(
    stage_cfg: Dict[str, Any], metrics: Dict[str, float]
...
def evaluate_health(
    ...
    det_health_history: List[Dict[str, Any]],
) -> Tuple[bool, Dict[str, Any]]:
...
def evaluate_convergence(
    ...
    state: StageState,
    metrics: Dict[str, float],
    snapshot: Optional[LogSnapshot] = None,
) -> Tuple[bool, Dict[str, Any]]:
    ...
    primary_metric = (
        "det_mAP50_pc"
        if "det_mAP50_pc" in metrics
        else ("det_mAP50" if "det_mAP50" in metrics else "combined")
    )
...
def evaluate_stability(
    stage_cfg: Dict[str, Any], snapshot: LogSnapshot, liveness_summary: Dict[str, Any]
) -> Tuple[bool, Dict[str, Any]]:
...
def evaluate_all_checklists(
    ...
    health_pass, health_details = evaluate_health(
        stage_cfg, snapshot, liveness_summary, det_health_history
    )
...
def get_retry_recommendation(state: StageState, why_result: Dict[str, Any]) -> str:
    ...
    msg = (
        f"Retry #{state.retry_count + 1} — strategy: {strategy['name']} ({strategy['description']})"
    )
...
def cmd_check() -> None:
    ...
    if state.retry_count > 0:
        logger.info(
            f"Retry #{state.retry_count} | Strategy: {state.current_strategy} | "
    ...
    if snapshot.det_health_lines:
        ...
        logger.info(
            f"DET-HEALTH: cls_mean={latest_dh['cls_mean']:.4f} "
    ...
    if snapshot.det_debug_lines:
        ...
        if latest_dd["floor_frac"] > 0.5:
            logger.warning(
                f"DET-DEBUG: floor_frac={latest_dd['floor_frac']:.2%} — many near-zero logits"
            )
    ...
    if TARGET_MET_FILE.exists():
        ...
        state.stage_history.append(
            {
                ...
                "checklist": {
                    "gate": {"passed": True, "details": signal_data.get("gate_details", {})}
                },
                "timestamp": datetime.now(timezone.utc).isoformat(),
            }
        )
        ...
        if not _verify_next_stage_has_resume_source(next_stage):
            logger.warning(
                "  Next stage resume source missing — creating symlink to current best.pth"
            )
            if BEST_CKPT.exists():
    ...
    if training_alive:
        ...
        if metrics:
            logger.info(
                f"Val metrics: det_mAP50={metrics.get('det_mAP50', 'N/A'):.4f}, "
        ...
        if snapshot.liveness.get("total_checks", 0) > 0:
            logger.info(
                f"Liveness: det={snapshot.liveness.get('det_alive_ratio', 0):.0%} "
        ...
        if metrics and snapshot.epoch > 0:
            if (
                not state.metric_history
                or state.metric_history[-1].get("epoch", -1) != snapshot.epoch
            ):
        ...
        checklist = evaluate_all_checklists(
            stage_cfg, state, metrics, snapshot, snapshot.liveness, state.det_health_history
        )
        ...
        for cat in ["gate", "health", "convergence", "validation", "stability"]:
            ...
            if not c.get("passed", True) and c.get("details"):
                for detail_key, detail_val in c["details"].items():
                    if isinstance(detail_val, dict) and detail_val.get("status") in (
                        "FAIL",
                        "WARN",
                    ):
        ...
        if dynamic_max_epochs != base_max_epochs:
            logger.info(
                f"  [DYNAMIC] max_epochs adjusted: {dynamic_max_epochs} (base: {base_max_epochs})"
            )
        ...
        if progress["eta"] != "N/A":
            logger.info(
                f"  Progress: {progress['epoch']} ({progress['progress_pct']:.0f}%) "
        ...
        if training_alive and state.epoch <= 1 and snapshot.det_health_lines:
            ...
            collapsed = (
                dh_cls_mean < -10 or dh_near_zero > 0.8 or latest_dh.get("cls_std", 1) < 0.05
            )
            if collapsed:
                logger.warning(
                    f"⚠ EARLY COLLAPSE DETECTED (epoch {snapshot.epoch}): "
        ...
        if decision == "continue":
            ...
            if cross_strategy and state.retry_count == 0:
                ...
                logger.info(
                    f'  [CROSS-STAGE] Using strategy "{cross_strategy}" from similar prior stage'
                )
            else:
            ...
            if should_escalate(state):
                ...
                logger.error(
                    f"  Last failure: {why['findings'][0]['title'] if why['findings'] else 'unknown'}"
                )
                state.status = "failed"
                save_state(state)
                return
            ...
            next_stage = RF_STAGES[next_idx]
            logger.info(
                f"(Near-gate) advancing to {next_stage['name']}: {next_stage['description']}"
            )
            if not _verify_next_stage_has_resume_source(next_stage):
        ...
        if state.status in ("failed", "aborted"):
            logger.info(
                f"Stage is {state.status} — waiting for human intervention or --reset to restart."
            )
            save_state(state)
            return
        ...
        if had_previous_run:
            ...
            state.current_strategy = select_retry_strategy(state)["name"]
            logger.warning(
                f"Training had previous run — retry #{state.retry_count} "
...
def _determine_resume_source(
    state: StageState, stage_cfg: Dict[str, Any], retry: bool = False
) -> Optional[Path]:
    ...
    if retry:
        ...
        if ckpt:
            logger.warning(
                f"Resume source: retry — {stage_name}/{resume_source}.pth not found,"
        ...
        if ckpt:
            logger.warning(
                f"Resume source: retry — {resume_source}.pth not found anywhere,"
    ...
    if state.stage_index > 0 and stage_name != RF_STAGES[0]["name"]:
        ...
        if ckpt:
            logger.warning(
                f"Resume source: {prev_name}/{prev_source}.pth not found,"
        ...
        if ckpt:
            logger.warning(
                f"Resume source: {prev_source}.pth not found anywhere,"
                f" falling back to shared {prev_fallback}.pth → {ckpt}"
            )
            return ckpt

        logger.warning(
            f"No checkpoint found for resume from {prev_name}"
...
def _verify_next_stage_has_resume_source(next_stage: Dict[str, Any]) -> bool:
    ...
    if prev_idx >= 0:
        ...
        logger.warning(
            f"Pre-advance check: no checkpoint for {next_stage['name']} "
...
def _launch_current_stage(
    ...
    if lock_pid is not None and is_pid_alive(lock_pid):
        ...
        try:
            ...
            matching_pids = (
                [int(p) for p in result.stdout.strip().split()] if result.stdout.strip() else []
            )
            if lock_pid in matching_pids:
                logger.warning(
                    f"Training already running (PID={lock_pid}) — skipping duplicate launch."
                )
                state.training_pid = lock_pid
    ...
    if existing:
        logger.warning(
            f"Found {len(existing)} existing training process(es) — killing before launch."
        )
...
def _print_paper_results(state: StageState) -> None:
    ...
    det_map50_95 = final_metrics.get(
        "det_mAP50:0.95",
        final_metrics.get("det_mAP50_95", final_metrics.get("det_mAP_50_95", "N/A")),
    )
...
def cmd_status() -> None:
    ...
    logger.info(
        f"Training PID   : {state.training_pid} {'(alive)' if is_pid_alive(state.training_pid) else '(dead)'}"
    )
    ...
    if cr and any(
        cr.get(c, {}).get("details")
        for c in ["gate", "health", "convergence", "validation", "stability"]
    ):
        logger.info("\nChecklist results:")
        for cat in ["gate", "health", "convergence", "validation", "stability"]:
            c = cr.get(cat, {})
    ...
    if state.det_health_history:
        ...
        logger.info(
            f"  cls_mean={latest_dh['cls_mean']:.4f}  cls_std={latest_dh['cls_std']:.4f}  "
    ...
    if state.stage_history:
        ...
        for h in state.stage_history:
            logger.info(
                f"  {h['stage']}: {h.get('epochs_completed', '?')} epochs, "
...
def main():
    ...
    parser.add_argument(
        "--check", action="store_true", help="Evaluate current stage and act (cron mode)"
    )
