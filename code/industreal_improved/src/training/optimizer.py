...
def build_scheduler(optimizer: torch.optim.Optimizer):
    ...
    if getattr(C, "ONE_CYCLE_LR", False):
        ...
        scheduler = CosineAnnealingWarmRestarts(optimizer, T_0=C.T_0, T_mult=C.T_mult, eta_min=1e-6)
    else:
        scheduler = CosineAnnealingLR(optimizer, T_max=C.EPOCHS - C.WARMUP_EPOCHS, eta_min=1e-6)
