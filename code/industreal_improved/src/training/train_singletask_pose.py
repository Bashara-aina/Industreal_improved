#!/usr/bin/env python3
"""
Single-task head-pose-only ConvNeXt-Tiny training (Opus 141, 4th baseline).

Patches src.config to enable ONLY the head-pose head (TRAIN_HEAD_POSE=True,
TRAIN_DET=False, TRAIN_ACT=False, TRAIN_PSR=False), then delegates all
training logic to train.py. All train.py CLI arguments are forwarded.

Purpose: produces a clean same-architecture pose-only denominator for multi-task
...
Usage:
    python src/training/train_singletask_pose.py [train.py args...]

Examples:
    # Fresh start from COCO-pretrained backbone:
    python src/training/train_singletask_pose.py --batch-size 2 --epochs 5 --lr 5e-4
...
train.CFG_TRAIN_HEAD_POSE = True
