        ...
        if result.returncode == 0 and result.stdout.strip():
            ...
            if len(parts) >= 3:
                return {
                    ...
                    "mem_util_pct": float(parts[1].strip()) / float(parts[2].strip()) * 100
                    if float(parts[2].strip()) > 0
                    else 0,
    ...
    try:
        ...
        if result.returncode == 0:
            for line in result.stdout.strip().split("\n"):
                if (
                    "train.py" in line
                    and "training_supervisor" not in line
                    and "stage_manager" not in line
                ):
                    return True
    except Exception:
        pass
...
def check_cooldown(state: Dict) -> bool:
    ...
    if elapsed_min < MIN_INTERVAL_MINUTES and last_run > 0:
        logger.info(
            f"[COOLDOWN] {elapsed_min:.0f}m < {MIN_INTERVAL_MINUTES}m — skipping (next check at "
...
SYMPTOM_DATABASE = [
    {
        ...
        "detect": lambda dd: any(
            d.get("cls_mean", 0) < CLS_MEAN_CRITICAL for d in dd if d.get("cls_mean") is not None
        ),
        "severity": "CRITICAL",
        "whys": [
        ...
        "fixes": [
            {
                ...
                "key": "MIXED_PRECISION",
                "value": "False",
                "comment": "Disable AMP — PSR seq loss spikes corrupt GradScaler",
            },
            {
                "type": "config",
                "key": "GRAD_CLIP_NORM",
                "value": "1.0",
                "comment": "Ensure gradient clipping is active",
            },
            {
                "type": "config",
                "key": "PSR_WEIGHT",
                "value": "10.0",
                "comment": "Reduce PSR weight to prevent backbone disruption",
            },
    ...
    },
    {
        ...
        "fixes": [
            {
                "type": "config",
                "key": "DET_LR_MULTIPLIER",
                "value": "10.0",
                "comment": "Double det LR to escape degenerate region",
            },
    ...
    },
    {
        ...
        "fixes": [
            {
                "type": "config",
                "key": "PSR_SEQ_LOSS_SCALE",
                "value": "3.0",
                "comment": "Double seq loss scale to push PSR out of zero region",
            },
            {
                "type": "config",
                "key": "PSR_WARMUP_STEPS",
                "value": "2000",
                "comment": "Half warmup to activate PSR earlier",
            },
        ...
        "fixes": [
            {
                ...
                "key": "PSR_WEIGHT",
                "value": "50.0",
                "comment": "Increase PSR weight to amplify signal",
            },
            {
                "type": "config",
                "key": "PSR_SEQ_LOSS_SCALE",
                "value": "3.0",
                "comment": "Amplify seq loss to overcome zero-loss equilibrium",
            },
            {"type": "restart", "retry_strategy": "default"},
        ],
    },
    {
        ...
        "detect": lambda _: (
            any("CUDA out of memory" in l for l in tail_log(SUBPROCESS_LOG, 100))
        ...
        "fixes": [
            {
                "type": "config",
                "key": "USE_BACKBONE_CHECKPOINT",
                "value": "True",
                "comment": "Enable gradient checkpointing to reduce memory",
            },
            {"type": "restart", "retry_strategy": "reduce_lr_5x"},
        ],
    },
        ...
        "fixes": [
            {
                ...
                "key": "GRAD_CLIP_NORM",
                "value": "0.5",
                "comment": "Tighter gradient clipping to dampen spikes",
            },
            {
                "type": "config",
                "key": "PSR_WEIGHT",
                "value": "15.0",
                "comment": "Reduce PSR weight for stability",
            },
            {"type": "restart", "retry_strategy": "reduce_lr_5x"},
        ],
    },
        ...
        "fixes": [
            {
                ...
                "key": "WARMUP_EPOCHS",
                "value": "10",
                "comment": "Extended warmup for better convergence",
            },
...
CLS_MEAN_CRITICAL = (
    -18.0
)  # collapse threshold (verified non-collapse at -12 with 237 alive anchors)
...
def assess_det_head(det_debug: List[Dict]) -> Tuple[str, Optional[float], str]:
    ...
    if cls_mean < CLS_MEAN_CRITICAL:
        return (
            "COLLAPSED",
            cls_mean,
            f"cls_mean={cls_mean:.3f} < {CLS_MEAN_CRITICAL} at step {step}",
        )
...
def main():
    ...
    if not pid_alive and other_running:
        logger.warning(
            f"[CONFLICT] Stale PID {pid} but other train.py process(es) found — cleaning up"
        )
        kill_any_training()
    ...
    logger.info(
        f"Stage: {stage} [{status}]  PID: {pid} [{'ALIVE' if pid_alive else 'DEAD'}]"
    ...
    gpu = get_gpu_info()
    logger.info(
        f"GPU: {gpu['gpu_util_pct']:.0f}% util  {gpu['mem_used_mib']:.0f}/{gpu['mem_total_mib']:.0f} MiB  "
        f"{gpu['temp_c']:.0f}°C"
    )
    ...
    if det_debug:
        ...
        logger.info(
            f"DET-DEBUG: step={latest_dd['step']} cls_mean={latest_dd['cls_mean']:.3f} "
    ...
    if det_health:
        ...
        logger.info(
            f"DET-HEALTH: step={latest_dh['step']} cls_mean={latest_dh['cls_mean']:.3f} "
    ...
    if psr_diag:
        ...
        logger.info(
            f"PSR: loss={latest_psr['loss']:.3e} logits_mean={latest_psr['logits_mean']:.1f} "
    ...
    if pid_alive:
        ...
        if symptoms:
            ...
            for s in symptoms:
                logger.warning(
                    f"[SYMPTOM] {s['id']}: {s['description']} (severity={s['severity']})"
                )
    ...
    if pid_alive:
        logger.info(
            f"Status: RUNNING  Epoch {epoch} step {step}/{total_steps}  "
    ...
    if pid_alive and det_cls_mean is not None:
        ...
        logger.info(
            f"Progress: epoch {epoch} step {step}/{total_steps} ({progress_pct:.0f}%)  "
    ...
    save_state(state)
    logger.info(
        f"[COOLDOWN] Next check allowed after {datetime.fromtimestamp(time.time() + MIN_INTERVAL_MINUTES * 60).strftime('%H:%M')}"
    )
