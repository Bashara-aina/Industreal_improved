...
class EmbeddingCache:
    ...
    def add_frame(self, rec_id: str, proj_feat: torch.Tensor, det_conf: torch.Tensor,
                  c5_gap: torch.Tensor, p4_gap: torch.Tensor,
                  activity: int, psr: torch.Tensor, frame_idx: int, camera: str):
    ...
    def flush_recording(self, rec_id: str):
        ...
        for key, dtype in [
            ('proj_feat', np.float16), ('det_conf', np.float16),
            ('c5_gap', np.float16), ('p4_gap', np.float16),
        ]:
    ...
    def total_frames(self) -> int:
        if self.mode == 'read':
            return sum(g.attrs['num_frames'] for g in
                       self._file['recordings'].values())
...
class CacheDataset(Dataset):
    ...
    def __init__(self, cache_dir: str, seq_len: int = 64, stride: int = 8,
                 task_mode: str = 'all', split: str = 'train'):
        ...
        logger.info(f"[CacheDataset] {len(self._index)} sequences, "
...
class CacheTrainer:
    ...
    def __init__(self, ckpt_path: str, cache_dir: str, output_dir: str,
                 seq_len: int = 64, batch_size: int = 128, lr: float = 1e-3):
    ...
    def _load_heads(self):
        ...
        logger.info(f"[CacheTrainer] Loaded {len(self.act_head_state)} activity params, "
    ...
    def train_heads(self, num_epochs: int = 50):
        ...
        train_ds = CacheDataset(self.cache_dir, seq_len=self.seq_len,
                                task_mode='all', split='train')
        val_ds = CacheDataset(self.cache_dir, seq_len=self.seq_len,
                              task_mode='all', split='val')
        ...
        logger.info(f"[CacheTrainer] Starting {num_epochs} epochs, "
        ...
        for epoch in range(num_epochs):
            ...
            with torch.no_grad():
                for batch in val_loader:
                    ...
                    psr_pred = (torch.sigmoid(psr_logits[..., :C.NUM_PSR_COMPONENTS]) > 0.3).float()
        ...
        logger.info(f"[CacheTrainer] Done. Best act_acc={best_act_acc:.4f}, "
                    f"best_psr_f1={best_psr_f1:.4f}")


# ============================================================================
# Main: Cache embeddings from a trained checkpoint
# ============================================================================
def cache_embeddings(ckpt_path: str, output_dir: str, split: str = 'val',
                     max_batches: Optional[int] = None, subset_ratio: float = 1.0):
    ...
    with torch.no_grad():
        for batch in tqdm(loader, total=max_batches or len(loader)):
            ...
            for i in range(images.shape[0]):
                ...
                activity = targets.get('activity', torch.zeros(images.shape[0], dtype=torch.long).to(device))[i].item()
                psr = targets.get('psr_labels', torch.zeros(images.shape[0], C.NUM_PSR_COMPONENTS).to(device))[i]
...
if __name__ == '__main__':
    ...
    ap.add_argument('--output-dir', type=str, default='runs/cache_training', help='Output for trained heads')
    ...
    if args.cache:
        ...
        trainer = CacheTrainer(
            args.ckpt, args.cache_dir, args.output_dir,
