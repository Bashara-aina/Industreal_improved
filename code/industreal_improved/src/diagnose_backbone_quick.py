...
ckpt = torch.load(SRC / 'runs' / 'rf_stages' / 'checkpoints' / 'crash_recovery.pth',
                  map_location='cpu', weights_only=False)
model = POPWMultiTaskModel(pretrained=False, backbone_type='convnext_tiny',
                           use_headpose_film=False, use_hand_film=False,
                           use_videomae=False, train_pose=False, use_backbone_checkpoint=False)
...
with torch.no_grad():
    ...
    for name, f_r, f_n in [("c2", c2_r, c2_n), ("c3", c3_r, c3_n),
                            ("c4", c4_r, c4_n), ("c5", c5_r, c5_n)]:
        f_r = f_r.float()
        f_n = f_n.float()

        # Mean/std
        print(f"\n  {name}:")
        print(f"    REAL:   μ={f_r.mean():.4f} σ={f_r.std():.4f} "
              f"spatial_std={f_r.std(dim=[2,3]).mean():.4f}")
        print(f"    RANDOM: μ={f_n.mean():.4f} σ={f_n.std():.4f} "
        ...
        corr = torch.stack([
            torch.corrcoef(torch.stack([
                f_r[0, ch_idx].flatten(),
                f_n[0, ch_idx].flatten()
            ]))[0, 1]
    ...
    pyramid_r = model.fpn(cfg._sanitize(c3_r) if hasattr(cfg, '_sanitize') else c3_r,
                          c4_r, c5_r)  # simplified
