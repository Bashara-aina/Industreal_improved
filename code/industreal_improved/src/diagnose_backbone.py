...
state_key = 'model_state_dict' if 'model_state_dict' in ckpt else \
            'model' if 'model' in ckpt and isinstance(ckpt['model'], dict) else None
if state_key:
    state_dict = ckpt[state_key]
    print(f"Using key: '{state_key}' — {len(state_dict)} params")
else:
    state_dict = {k: v for k, v in ckpt.items() if any(k.startswith(p) for p in ['backbone.', 'fpn.', 'detection_head.'])}
    if not state_dict:
...
with torch.no_grad():
    ...
    for name, feat in [('c2', c2), ('c3', c3), ('c4', c4), ('c5', c5)]:
        ...
        print(f"    channel mean:  μ={ch_mean.mean().item():.6f}  σ={ch_mean.std().item():.6f}  range=[{ch_mean.min().item():.6f}, {ch_mean.max().item():.6f}]")
        print(f"    channel std:   μ={ch_std.mean().item():.6f}  σ={ch_std.std().item():.6f}  range=[{ch_std.min().item():.6f}, {ch_std.max().item():.6f}]")

        # Spatial variance (is the feature map flat?)
        spatial_std = f.std(dim=[2,3]).mean(dim=0)
        print(f"    spatial std per channel: μ={spatial_std.mean().item():.6f}  σ={spatial_std.std().item():.6f}")
    ...
    for name in ['p3', 'p4', 'p5', 'p6', 'p7']:
        if name in pyramid:
            ...
            print(f"  {name}: shape={list(f.shape)}  mean={f.mean().item():.6f}  std={f.std().item():.6f}  "
    ...
    print(f"    sigmoid max: mean={cls_max.mean().item():.6f}  std={cls_max.std().item():.6f}  "
    ...
    print(f"    per-anchor class std: μ={cls_flat_std.mean().item():.6f}  "
    ...
    print(f"    cls_mean: {cls_mean_val:.6f}  (equivalent sigmoid: {torch.sigmoid(cls.mean()).item():.6f})")
...
if c5_std < 0.01:
    ...
    print("  [WARNING] Backbone C5 variance very low ({:.4f}) — features may be too weak".format(c5_std))
else:
...
if abs(bg_mean - fg_mean) < 0.1:
    print("  [WARNING] Background/foreground scores nearly identical — classifier not discriminating")
