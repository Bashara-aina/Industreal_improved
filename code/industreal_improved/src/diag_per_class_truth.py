...
def has_per_class(val: Dict[str, Any]) -> bool:
    return bool(
        val.get("det_per_class") or val.get("det_per_class_ap") or val.get("det_per_class_gt")
    )
...
def fmt_table(rows: List[Dict[str, Any]]) -> str:
    lines = [
        f"  {'ch':>2}  {'cat':>3}  {'name':<14} {'GT':>6}  {'AP':>6}  status",
        f"  {'--':>2}  {'---':>3}  {'-' * 14} {'-' * 6}  {'-' * 6}  ------",
    ]
    for r in rows:
        if r["is_background"]:
...
def main() -> int:
    ...
    if not det_recs:
        ...
        print(
            "(Per-class AP is written from the eval that runs every DET_METRICS_EVERY_N "
    ...
    if args.epoch is not None:
        ...
        if chosen is None:
            print(
                f"No per-class record for epoch {args.epoch}; "
    ...
    print(
        f"  channels: {len(present)} present | {n_zero_gt} zero-GT | {n_bg} background | "
    ...
    if ch6:
        ...
        if ch6["gt"] == 0:
            print(
                '  -> zero GT in this eval: UNMEASURABLE, not a "1739-GT mystery". '
                "The v11 §5 count was an index/source error."
            )
        elif ch6["ap"] == 0.0:
            print(
                "  -> genuinely STUCK (has GT, AP=0). Compare its GT count above to the "
