...
print(
    f"[quick_eval]   GT class histogram (top 10): {total_gt_per_class.most_common(10)}", flush=True
)
print(f"[quick_eval]   Pred class histogram (top 10): {_cls_dist.most_common(10)}", flush=True)
if _score_dist:
    print(
        f"[quick_eval]   Post-NMS score stats: n={len(_score_dist)} min={min(_score_dist):.4f} max={max(_score_dist):.4f} mean={sum(_score_dist) / len(_score_dist):.4f}",
        flush=True,
    )
...
print(
    f"[quick_eval]   GT classes in batch: {len(gt_set)}; Pred classes: {len(pred_set)}; overlap: {len(overlap)}",
    flush=True,
)
if len(overlap) < 5:
