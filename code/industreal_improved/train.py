...
def main(args):
    ...
    if args.resume:
        ...
        try:
            ...
            logger.warning(
                f'  Could not restore scheduler/scaler ({e}). '
                f'Re-initialized -- LR schedule continues.'
            )
        ...
        if start_epoch < C.WARMUP_EPOCHS:
            ...
            logger.info(
                ...
                'det=0.0  pose=-1.0  act=0.0  psr=0.0'
            )
        else:
            logger.info(
            ...
            if (epoch + 1) % C.VAL_EVERY == 0:
                ...
                while True:
                    ...
                    try:
                        ...
                        logger.error(
                            f'  Val attempt {val_attempt}/4 FAILED: '
                            f'{_exc_name}: {_exc_msg}'
                        )
                        # DEBUG: log what type of exception and what happens next
                        ...
                        if not (is_cpu_enomem or is_cuda_oom_v):
                            logger.error(
                                f'  Non-OOM exception in evaluate_all — aborting train.py immediately.'
                            )
                            raise
                        # OOM path: retry with reduced settings
                        if is_cuda_oom_v:
                            logger.warning('Validation CUDA OOM — retrying with reduced settings.')
                        else:
                            logger.warning('Validation ENOMEM — retrying with reduced settings.')
                        val_batch_size_rt = max(1, val_batch_size_rt // 2)
                        val_workers_rt = 0
                        val_prefetch_rt = 1
                        val_max_batches_rt = max(1, int(val_max_batches_rt) // 2)
                        logger.warning(
                            f'  Val retry {val_attempt}/4: '
                            f'batch={val_batch_size_rt} workers={val_workers_rt} '
                            f'prefetch={val_prefetch_rt} max_batches={val_max_batches_rt}'
                        )
                        gc.collect()
                        torch.cuda.empty_cache()
                        continue
                        ...
                        gc.collect()
                        torch.cuda.empty_cache()
                    break  # exit while loop on successful evaluation
                    ema.restore()
                ...
                _task_keys = ('act_macro_f1', 'psr_macro_f1')
                _task_nan = any(
