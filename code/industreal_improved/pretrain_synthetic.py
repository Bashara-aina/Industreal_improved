...
NUM_DET_CLASSES = 24
PRETRAIN_BATCH_SIZE = 16  # Safe for full multi-task model (all heads run forward even in det-only pretrain); 5GB VRAM peak
PRETRAIN_NUM_WORKERS = 4  # Fewer workers = less HDD seek contention on rotational disk
PRETRAIN_GRAD_ACCUM_STEPS = 4  # Was 16; fewer steps = 4× more optimizer updates per epoch = faster convergence + lower memory pressure
    ...
    def __getitem__(self, idx):
        ...
        if self.augment and random.random() > 0.5:
            ...
            for ann in sample['annotations']:
                ...
                new_x1 = self.img_size[0] - x2
                new_x2 = self.img_size[0] - x1
                ann['bbox'] = [new_x1, y1, new_x2, y2]
        ...
        for ann in sample['annotations']:
            x1, y1, w, h = ann['bbox']
            x2, y2 = x1 + w, y1 + h
            x1 = max(0, min(x1, self.img_size[0] - 1))
            y1 = max(0, min(y1, self.img_size[1] - 1))
            x2 = max(0, min(x2, self.img_size[0]))
...
def main(args, skip_val: bool = False):
    ...
    train_ds = DetectionPretrainDataset(
        ...
        augment=True,
        max_recordings=int(getattr(C, 'PRETRAIN_MAX_TRAIN_RECORDINGS', None)),
    )
    ...
    epochs = int(getattr(C, 'PRETRAIN_DET_EPOCHS', 20))
    ...
    for epoch in range(start_epoch, epochs):
        ...
        train_metrics = train_one_epoch_det(
            ...
            accum_steps=PRETRAIN_GRAD_ACCUM_STEPS,
        )
        scheduler.step()
        ...
        if skip_val:
            pass  # Skip validation during fast pretrain; use evaluate.py for full mAP
        elif (epoch + 1) % 5 == 0 or epoch == epochs - 1:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--skip-val', action='store_true', help='Skip validation (faster; use evaluate.py for full eval)')
    args = parser.parse_args()
    main(args, skip_val=args.skip_val)