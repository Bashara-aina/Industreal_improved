...
def run_multi_seed_evaluation(
    ...
    metric_keys = [
        'act_accuracy', 'act_macro_f1', 'act_clip_accuracy', 'act_top5_accuracy',
        'head_pose_MAE', 'forward_angular_MAE_deg', 'up_angular_MAE_deg', 'position_MAE_mm',
        'psr_overall_f1', 'psr_f1_at_t5', 'psr_precision_at_t5', 'psr_recall_at_t5',
        'psr_f1_at_t3', 'psr_precision_at_t3', 'psr_recall_at_t3',
...
def compute_psr_metrics(
    ...
    for c in range(num_components):
        ...
        f1_at_t = 2 * prec_t * rec_t / (prec_t + rec_t) if (prec_t + rec_t) > 0 else 0.0
        f1_at_t_values.append(f1_at_t)
        prec_at_t_values.append(prec_t)
        rec_at_t_values.append(rec_t)

    psr_f1_at_t = float(np.mean(f1_at_t_values)) if f1_at_t_values else 0.0
    psr_precision_at_t = float(np.mean(prec_at_t_values)) if prec_at_t_values else 0.0
    psr_recall_at_t = float(np.mean(rec_at_t_values)) if rec_at_t_values else 0.0
        ...
        try:
            with torch.no_grad():
                gflops, _ = thop.profile(
                    model, inputs=(dummy_img,), verbose=False,
                )
            gflops = gflops / 1e9
        except Exception:
            gflops = float('nan')
...
def evaluate_all(
    ...
    if _in_eval:
        raise RuntimeError(
            'RE-ENTRY BUG: evaluate_all called while already running '
            '(model._evaluate_all_active=True). Aborting to prevent double execution.'
        )
    ...
    try:
        ...
        _eval_start = time_module.perf_counter()
        logger.info(
            f'  [evaluate_all] batch_iterations={max_batches} '
            f'flip_tta={use_flip_tta} crop_tta={use_crop_tta}'
        )

        _progress_interval = max(1, max_batches // 10)
        for bi, (images, targets) in enumerate(loader):
            if bi >= max_batches:
                break
            if bi > 0 and bi % _progress_interval == 0:
                logger.info(f'  [evaluate_all] batch {bi}/{max_batches}')
        ...
        if 'hand_joints' in targets:
            targets['hand_joints'] = targets['hand_joints'].to(device)
        if 'box_mask' in targets:
            targets['box_mask'] = {k: v.to(device) for k, v in targets['box_mask'].items()}
        ...
        if use_flip_tta:
            ...
            for key in ['act_logits', 'psr_logits']:
                if key in out_flip and out_flip[key].dim() >= 3:
                    outputs_raw[key] = 0.5 * (outputs_raw[key] + torch.flip(out_flip[key], dims=[2]))
                elif key in out_flip:
                    outputs_raw[key] = 0.5 * (outputs_raw[key] + out_flip[key])
        ...
        for i in range(B):
            ...
            rec_id = metadata_list[i].get('recording_id', None) if i < len(metadata_list) else None
    ...
    # -------------------------------------------------------------------------
    # PSR Metrics — evaluated at both tolerances per benchmark protocol
    # -------------------------------------------------------------------------
    logger.info('  ── PSR ──')
    ...
    psr_metrics_t5 = compute_psr_metrics(all_psr_logits, all_psr_labels, tolerance_frames=5)
    psr_metrics_t3 = compute_psr_metrics(all_psr_logits, all_psr_labels, tolerance_frames=3)

    results['psr_overall_f1'] = psr_metrics_t5['psr_overall_f1']
    results['psr_edit_score'] = psr_metrics_t5['psr_edit_score']
    results['psr_pos'] = psr_metrics_t5['psr_pos']
    results['psr_f1_at_t5'] = psr_metrics_t5['psr_f1_at_t']
    results['psr_precision_at_t5'] = psr_metrics_t5['psr_precision_at_t']
    results['psr_recall_at_t5'] = psr_metrics_t5['psr_recall_at_t']
    results['psr_f1_at_t3'] = psr_metrics_t3['psr_f1_at_t']
    results['psr_precision_at_t3'] = psr_metrics_t3['psr_precision_at_t']
    results['psr_recall_at_t3'] = psr_metrics_t3['psr_recall_at_t']

    logger.info(
        f'  PSR — Overall F1: {results["psr_overall_f1"]:.4f}  '
        f'F1@T5: {results["psr_f1_at_t5"]:.4f}  P@T5: {results["psr_precision_at_t5"]:.4f}  R@T5: {results["psr_recall_at_t5"]:.4f}  '
        f'F1@T3: {results["psr_f1_at_t3"]:.4f}  P@T3: {results["psr_precision_at_t3"]:.4f}  R@T3: {results["psr_recall_at_t3"]:.4f}  '
        f'Edit: {results["psr_edit_score"]:.4f}  POS: {results["psr_pos"]:.4f}'
    )

    # -------------------------------------------------------------------------
    # Assembly State Recognition Metrics (Paper 8 — IEEE RAL 2024)
    # -------------------------------------------------------------------------
    logger.info('  ── Assembly State ──')
    ...
    # -------------------------------------------------------------------------
    logger.info('  ── Error Verification ──')
    ...
    gt_box_total = int(sum(dg_boxes[i].shape[0] for i in range(len(dg_boxes))))
    if gt_box_total == 0:
    ...
    results['eff_params_m'] = float(_total_params / 1e6)
    results['eff_gflops'] = float('nan')
    results['eff_fps'] = float('nan')
    results['eff_resolution'] = f'{C.IMG_HEIGHT}x{C.IMG_WIDTH}'

    model.train()
    if hasattr(model, '_evaluate_all_active'):
        model._evaluate_all_active = False
    _elapsed = time_module.perf_counter() - _eval_start
    logger.info(
        f'  [evaluate_all] DONE in {_elapsed:.1f}s '
        f'(batch_iterations={max_batches})'
    )
    return results
...
def _print_multi_seed_summary(summary: Dict[str, Any]) -> None:
    ...
    metric_keys = [
        ...
        ('act_top5_accuracy', 'Activity Top-5 Acc'),
        ...
        ('psr_f1_at_t5', 'PSR F1@T5'),
        ('psr_precision_at_t5', 'PSR P@T5'),
        ('psr_recall_at_t5', 'PSR R@T5'),
        ('psr_f1_at_t3', 'PSR F1@T3'),
        ('psr_precision_at_t3', 'PSR P@T3'),
        ('psr_recall_at_t3', 'PSR R@T3'),
...
def _print_single_run_results(results: Dict[str, Any], split: str) -> None:
    ...
    print(f'  Forward angular MAE   : {results["forward_angular_MAE_deg"]:.4f} deg')
    print(f'  Up angular MAE        : {results["up_angular_MAE_deg"]:.4f} deg')
    print(f'  Position MAE           : {results["position_MAE_mm"]:.4f} mm')
    ...
    print(f'  Precision@T (±5)      : {results["psr_precision_at_t5"]:.4f}')
    print(f'  Recall@T (±5)         : {results["psr_recall_at_t5"]:.4f}')
    print(f'  F1@T (±3 frames)       : {results["psr_f1_at_t3"]:.4f}')
    print(f'  Precision@T (±3)      : {results["psr_precision_at_t3"]:.4f}')
    print(f'  Recall@T (±3)         : {results["psr_recall_at_t3"]:.4f}')
