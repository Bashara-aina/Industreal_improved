...
class FocalLoss(nn.Module):
    ...
    def __init__(self, alpha: float = 0.25, gamma: float = 2.0,
                 pos_iou_thresh: float = 0.3, neg_iou_thresh: float = 0.4):
        super().__init__()
        self.alpha = alpha
        self.gamma = gamma
...
class ClassBalancedFocalLoss(nn.Module):
    ...
    def compute_beta_weights(self, frequencies: torch.Tensor) -> None:
        ...
        # Clamp max weight ratio to prevent catastrophic reweighting
        max_ratio = 50.0
        w_min = self.class_weights.min()
        ratios = self.class_weights / w_min.clamp(min=1e-8)
        clamp_mask = ratios > max_ratio
        if clamp_mask.any():
            self.class_weights.data[clamp_mask] = w_min * max_ratio
            logger.debug(f'CB weights clamped: {clamp_mask.sum().item()}/{self.num_classes} classes')
    ...
    def forward(self, outputs: Dict, targets: Dict) -> Tuple[torch.Tensor, Dict]:
        ...
        for _loss, _name, _zero in [
            ...
            if not torch.isfinite(_loss).all():
                _fallback = torch.tensor(1.0, device=device, dtype=output_dtype) if _name in ('det', 'pose', 'psr', 'head_pose') else _zero
                if _name == 'det':
                    loss_det = _fallback
                elif _name == 'pose':
                    loss_pose = _fallback
        ...
        # Replace any NaN/inf with 1.0 — ensures exp(1)*1.0 + (-1) ≈ 1.718 > 0,
        # so Kendall total stays positive and gradient flows through log_vars.
        # [CRITICAL FIX 2026-06-12] _safe must PRESERVE the autograd graph.
        # Old version: `torch.tensor(1.0)` — detached constant, disconnects loss
        # from all model params when any component is NaN. After repeated NaN,
        # loss has no grad_fn → backward() crash.
        # New version: `torch.where(isfinite(l), l_or_clamped, 1.0)` — replaces
        # NaN values with 1.0 but keeps the graph connected. Gradient for NaN
        # positions flows through the 1.0 branch (constant → gradient = 0).
        _safe = lambda l, z: torch.where(
            ...
            torch.tensor(1.0, device=device, dtype=l.dtype)
        )
        ...
        # Also NaN-guard the log_vars: torch.clamp does NOT replace NaN
        # (IEEE 754: NaN comparisons are always False). If a log_var became
        # NaN from a previous backward, exp(-NaN) = NaN corrupts Kendall.
        if not torch.isfinite(self.log_var_det).all():
            self.log_var_det.data = torch.zeros_like(self.log_var_det)
        if not torch.isfinite(self.log_var_pose).all():
            self.log_var_pose.data = torch.zeros_like(self.log_var_pose)
        if not torch.isfinite(self.log_var_act).all():
            self.log_var_act.data = torch.zeros_like(self.log_var_act)
        if not torch.isfinite(self.log_var_psr).all():
            self.log_var_psr.data = torch.zeros_like(self.log_var_psr)
        ...
        if self.use_kendall:
            ...
            if bool(getattr(C, 'STAGED_TRAINING', True)) and self._current_epoch >= 1:
                ...
                if stage == 1:
                    ...
                    prec_psr = prec_psr * 0
                    lv_psr = lv_psr * 0
                # Stage 3 warmstart: PSR precision was zeroed for 15 epochs (stage 1+2).
                # Without warmstart, exp(-lv_psr) starts near zero (poor init) and would
                # collapse AS/EV immediately. Linear ramp avoids shock.
                elif stage == 3:
                    stage1_end = int(getattr(C, 'STAGE1_EPOCHS', 5))
                    stage2_end = stage1_end + int(getattr(C, 'STAGE2_EPOCHS', 10))
            ...
            if not math.isfinite(total_val):
                ...
                if finite_parts:
                    ...
                    total = loss_det
                # Reconnect to autograd through log_vars — _safe constants have no grad_fn
