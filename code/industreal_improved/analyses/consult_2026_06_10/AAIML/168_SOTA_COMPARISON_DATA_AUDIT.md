...
- PSR F1: 0.901 (CVIU 2025, STORM), 0.731 (B2), 0.883 (B3)
...
- Pose fwd 9.14° (95% CI 7.74–10.87°), up 7.78° (CI 6.89–8.81°) — full-38k bootstrap from bootstrap_ci.json
...
| `src/runs/rf_stages/checkpoints/d4_d1r/metrics.json` | D1R detection (YOLOv8m 0.995, Ultralytics-native val) |
...
| V5b multi-task (ConvNeXt) | 0.00009 | NOT comparable (different arch); MT ConvNeXt actually reaches 0.468 mAP50_pc at epoch 62 but full-38k eval needed |
| V8 multi-task (YOLOv8m shared) | TBD | TBD |

**Honest claim:** D1R YOLOv8m 0.995 (Ultralytics-native validation) exceeds WACV 0.838 on a like-architecture basis, but note: 0.995 is the framework-native metric (Ultralytics protocol), not our full-38k COCO-style eval — the two protocols differ by up to three orders of magnitude. V5b multi-task is too weak for direct comparison.
...
| Source | Number | Comparable? |
|---|---|---|
| STORM-PSR | 0.901 (CVIU 2025) | Different paradigm (transition detection). Also POS 0.812, transition tolerance τ=15.5s. |
...
| V5b multi-task | fwd 9.14° (CI 7.74–10.87°), up 7.78° (CI 6.89–8.81°) | First baseline (no SOTA to beat) |
...
- The actual numbers are in:
  - `d4_d1r/metrics.json` (D1R YOLOv8m fine-tuned 25 epochs from COCO init, Ultralytics-native mAP50=0.995; comparable to WACV but framework-native metric)
...
- The STORM 0.901 is from arXiv:2510.12385 (CVIU 2025), Table 1. POS 0.812, transition tolerance τ=15.5s.
...
| "PSR F1 0.7018" | V5b pre-fix per-comp opt | STORM 0.901 (CVIU 2025), B2 0.731 | **Different paradigm** (per-frame vs transition) |
| "Pose fwd 9.14° (CI 7.74–10.87°), up 7.78° (CI 6.89–8.81°)" | V5b multi-task (full-38k bootstrap) | None | First baseline, no SOTA |

## 6. Where Each Number Comes From (file paths, verifiable)

| Number | File | Line | Status |
|---|---|---|---|
| 0.995 mAP50 (D1R) | `src/runs/rf_stages/checkpoints/d4_d1r/metrics.json` | (D1R YOLOv8m fine-tuned 25 epochs from COCO init, Ultralytics-native val mAP50) | Verified |
...
| fwd 9.14° (CI 7.74–10.87°), up 7.78° (CI 6.89–8.81°) | `src/runs/rf_stages/checkpoints/bootstrap_ci.json` | (full-38k bootstrap) | Verified |
...
| Pose | fwd 9.14° (CI 7.74–10.87°), up 7.78° (CI 6.89–8.81°) | V5b full-38k bootstrap | First baseline (no SOTA) |
...
| Pose fwd MAE | 9.14° (CI 7.74–10.87°) | V5b full-38k bootstrap | **First baseline** (no SOTA) |
