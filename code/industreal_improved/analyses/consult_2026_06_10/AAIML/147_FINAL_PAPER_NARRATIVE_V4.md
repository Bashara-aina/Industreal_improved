...
**4.2 Detection 5 classes NEVER predicted.** The D3 multi-task detection head has 24 output channels but 5 classes (1, 13, 16, 19, 23) never fire at any confidence threshold. Root cause: either a class mapping bug between the detection head's logit ordering and the COCO-based class index, or an initialization defect that permanently silences those channels. The detection head also fires on wrong classes at high confidence, indicating insufficient per-class discriminative capacity.
