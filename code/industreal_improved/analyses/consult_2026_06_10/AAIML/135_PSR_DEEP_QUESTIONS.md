...
|---|---|---|
| `SOTA_STATUS.md` | Full PSR results table, per-comp breakdown, null-POS experiment, D4 verdict | macro F1=0.7018 (per-comp, 38k), 0.7810 (5k subset, superseded), 0.6788 (global 0.10, 38k) |
...
| `psr_optimal_thr/optimal_thresholds.json` | Per-comp optimal thresholds (10k frames, superseded by 38k) | optimal_macro_f1=0.7499 (10k; corrected 38k: 0.7018), global_0.10_macro_f1=0.7217 (10k; corrected 38k: 0.6788) |
...
- **What to verify:** Compute per-comp optimal thresholds on the 5k subset and compare to the full 38k thresholds. If they differ by >0.1 for any component, the threshold is sensitive to frame selection and a single optimal threshold is not a stable configuration. If they are identical, the 0.7810 vs 0.7018 difference is just evaluation randomness (the subset was easier), not a better threshold configuration.
...
**Q15. The macro F1 improvement from global 0.10 to per-comp optimal was +0.028 on the 10k subset (0.7217→0.7499). On the full 38k, the improvement is +0.023 (0.6788→0.7018). How much of this improvement comes from adjusting thresholds for comp4 and comp10 alone?**
...
- **Current answer:** STORM uses a 200M+ parameter architecture with 3D convolutions and temporal attention over 16-frame clips. Our model uses ConvNeXt-Tiny (28M backbone, ~50M total). The compute gap is ~4x in parameters and likely ~10x in FLOPs. If our per-comp optimal F1 (0.7018) is close to the compute-parameter envelope for a 50M model, then matching STORM's 0.901 requires a larger backbone. The question is: how much of the 0.151 F1 gap is architecture (fixable with larger model) and how much is paradigm (per-frame vs. transition-based training)?
...
- **Current answer:** The in-flight training bundles both fixes. If PSR F1 improves from 0.7018 to, say, 0.8000, we cannot attribute the +0.050 to head repair vs. fixed Kendall. A factorial ablation (head repair only, fixed Kendall only, both) would be needed. However, 133 PSR-3 states the evidence points at dead heads as the primary cause, not Kendall down-weighting (which is 4-8%). If the improvement is >+0.10, heads are the primary cause. If it's <+0.03, Kendall was the primary cause.
...
**Q49. STORM and B3 are evaluated on the full 38k validation set. Our per-comp optimal F1 was 0.7499 on 10k frames (corrected to 0.7018 on 38k). Does the 10k-to-38k gap affect comparability?**
...
- **Current answer:** The optimal thresholds were computed on 10k frames, but SOTA_STATUS.md reported the per-comp optimal F1 as 0.7499 from the 10k optimization (later corrected to 0.7018 on full 38k). The full 38k eval at global 0.10 gives macro F1=0.677 (from `full_eval_ep18_stream/metrics.json` L94). The per-comp optimal F1 on the full 38k has never been computed. If the 10k optimal thresholds perform worse on the full 38k (overfitting to the subset), the 0.7018 is an upper bound, not a representative number.
- **What to verify:** Apply the optimal thresholds (from `psr_optimal_thr/optimal_thresholds.json`) to all 38k frames and compute macro F1. If it's similar to 0.7018 (within ±0.01), the threshold is stable across frame subsets. If it's significantly lower (e.g., 0.70-0.72), the reported 0.7018 is an overestimate and the honest number is the full-eval figure.
...
- **Current answer:** We have three PSR improvement mechanisms that have never been combined: (1) per-comp optimal thresholds (+0.028 over global 0.10), (2) head repair (+unknown, expected +0.03-0.08), (3) fixed Kendall weights (+unknown, expected +0.01-0.03). The combined potential is 0.7018 (current) + repair_delta + kendall_delta. If repair gives +0.05 and Kendall gives +0.02, macro F1 could reach 0.82 — still below STORM's 0.901 but "approaching competitive" by the PW-3 rubric. If combined F1 exceeds 0.85, the paradigm gap narrative changes from "fundamentally different" to "bridgeable with architectural improvements."
...
**D3. Full 38k per-comp optimal F1 computation.** The original 0.7499 was on 10k frames. Full 38k eval at per-comp optimal thresholds yields the corrected 0.7018. Estimated compute: 30 min on RTX 3060 (eval of existing cached logits). Decision: run immediately on the RTX 3060 while the RTX 5060 Ti trains. This is zero-cost (doesn't interfere with training).
