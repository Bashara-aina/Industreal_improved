...
| 8 | PSR per-comp optimal F1 | 0.7018 (38k; originally 0.7499 on 10k subset) | **first-baseline** (per-frame PSR); *not* "competitive" (16.8% rel. gap to STORM, different paradigm → also **not-comparable**) | psr_optimal_thr ✔ | "Per-component-calibrated macro-F1 0.7499 (10k val subset, superseded); full-38k figure 0.7018 (val-selected); 38k global 0.10 = 0.6788" |
...
The originally reported 0.7499 was a 10k-frame val-selected upper bound (135 Q49; 135-debate Challenge 4); corrected full-38k per-comp optimal is 0.7018. Three numbers must appear in one table row: global-0.10, per-comp-optimal-38k, LOO-CV mean ± σ (138 Q2.06). If the 38k figure lands at 0.70-0.72, that is the honest primary and the STORM gap widens — report it anyway; committing in advance to publish whichever way it moves is the paper's strongest integrity signal (138 Q2.09). Reconcile in the same pass why the 38k stream at global 0.10 gave 0.6773 while the 10k gave 0.7217.
...
**Debate 2 (135 — PSR F1 validity): PARTIALLY RESOLVED, one part by this audit.** input_dim → moot (dead code, §-1e). 10k-vs-38k → run Day 1 (Q2). Attribution → dissolved: the in-flight run is single-factor Kendall-only (§-1b); the head repair, once actually wired, becomes the second single-factor run. What remains open is only whether the 0.7018 survives the 38k eval.
...
6. **PSR thresholds are validation-selected**: per-component-optimal macro-F1 = 0.7499 on a 10k-frame subset (superseded) vs 0.7217 at global 0.10 threshold; corrected full-38k: 0.7018 per-comp optimal vs 0.6788 global 0.10; leave-one-recording-out CV bounds the selection benefit at +0.0358 ± 0.0216 across 16 recordings. [Full-38k per-comp figure: X; LOO caveat: recordings span the model's train/val membership.]
