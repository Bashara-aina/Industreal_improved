...
| SOTA: act 0.652 (MViTv2-S), det 0.838 (YOLOv8m), PSR 0.901 (STORM), pose = **no SOTA** | ✅ | `176 §4.1` table, sourced to Schoonbeek et al. WACV 2024 |
| "MViTv2-S too weak / needs foundation model" (183 §1.3) | ❌ **REFUTED** | activity SOTA itself is MViTv2-S; det SOTA is a *smaller* CNN |
...
Two things I could **not** verify from this ephemeral container: the *live* Path-D run's post-ep6 metrics (the GPU run is on your machine, not here), and the exact per-task backbone the WACV paper's table lists (arXiv abstract/project page omit the tables). The activity=MViTv2-S attribution comes from **your own** 176 §4.1; treat the exact figure as "confirm against the PDF," but the strategic implication — *this dataset's activity SOTA is reachable with a backbone in the MViTv2-S class* — is robust.
