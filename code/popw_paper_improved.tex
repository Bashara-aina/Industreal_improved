...
% ---------- PDF metadata ----------
\title{\popw{}: A Unified Multi-Task Architecture for Assembly Understanding}
\author{Bashara Aina}
\date{}
...
% =====================================================================
% ABSTRACT — result-free draft (every number is a \popwres placeholder; fill after eval)
% =====================================================================
\begin{abstract}
...
Ablations isolate the contribution of multi-task sharing and of FiLM conditioning. \popw{}
shows that unified egocentric assembly understanding is feasible and efficient without
...
Assembly video datasets take two distinct forms. Egocentric assembly video (e.g., IndustReal~\cite{schoonbeek2024industreal}) captures footage from a head-mounted or body-worn camera, providing close-up hand-object views that make fine-grained state discrimination feasible but introduce severe self-occlusion and viewpoint instability. Third-person assembly video (e.g., IKEA ASM~\cite{benshabat2021ikea}) captures footage from a fixed external camera mounted above or beside the workspace, providing a broader overview of the assembler's actions but with reduced resolution on hand-object interactions and increased occlusion from the assembler's body. Both settings require the same four interpretable tasks — assembly state detection, body/head pose estimation, activity recognition, and procedure step recognition — but the visual characteristics of each task differ substantially between camera modalities.

Current approaches handle these tasks independently. The IndustReal benchmark~\cite{schoonbeek2024industreal} establishes YOLOv8m as the strongest detector for 24-class assembly state detection (ASD), achieving 83.80\% mAP@0.5 on the full test set (83.80\% bbox mAP@0.5, 64.1\% video-level mAP@0.5). Activity recognition relies on MViTv2 pretrained on Kinetics-400, achieving 65.25\% Top-1 on RGB-only and 66.45\% Top-1 with an RGB+VL+stereo ensemble~\cite{schoonbeek2024industreal}. Procedure step recognition employs the STORM-PSR dual-stream framework~\cite{schoonbeek2025storm}, which achieves POS=0.812 and F1=0.506 on IndustReal ($\tau$=15.5s), while the best baseline from the original PSRT paper (B2, combining ASD confidence accumulation with procedure-order constraints) achieves POS=0.816 and F1=0.731 on IndustReal~\cite{schoonbeek2024industreal}. For IKEA ASM, prior work establishes I3D-based methods for activity recognition and ActionFormer variants for temporal action localization (see Section~\ref{sec:datasets} for full details). This fragmentation means each deployment requires running multiple large models, with no sharing of visual features across tasks.

The multi-task learning paradigm offers an alternative. Recent work on hierarchical temporal transformers for egocentric pose and action understanding~\cite{wen2022hierarchical} explores sharing features across pose and action tasks. Similarly, MECCANO~\cite{ragusa2022meccano} uses multimodal egocentric datasets for joint modeling of behavior and environment in industrial settings. The key insight is that assembly tasks share structural dependencies: object states constrain possible actions, head pose indicates gaze direction and focus, and temporal progression constrains procedure steps.
...
\item Kendall homoscedastic uncertainty weighting combined with staged training enables stable joint optimization across heterogeneous task losses~\todo.
\item Evaluate \popw{} on IKEA ASM and IndustReal datasets~\todo: competitive accuracy against dedicated baselines, computational efficiency through weight sharing~\todo.
\end{enumerate}
...
\subsection{Video Understanding for Assembly}

Third-person (fixed-view) video presents distinct challenges compared to egocentric video: the camera is positioned external to the performer, providing a broader overview of the workspace but requiring different visual reasoning for hand-object interactions, occlusions, and action recognition~\cite{benshabat2021ikea}.
...
Recent advances in hierarchical temporal modeling for egocentric video explore joint modeling of hand pose and action recognition~\cite{wen2022hierarchical}. Similarly, Ego-Only models~\cite{wang2023ego} perform egocentric action detection without transferring from exocentric pretrained features, indicating that egocentric-specific representations are beneficial. STEPs~\cite{shah2023steps} extends this by learning self-supervised temporal perception from unlabeled procedural videos, enabling multi-view online action detection without explicit labeled training data for temporal structure.
...
\textbf{Activity Recognition:} For activity recognition on IndustReal, MViTv2 (Kinetics-400 pretrained) achieves 65.25\% Top-1 and 87.93\% Top-5 on RGB-only, improving to 66.45\% Top-1 and 88.43\% Top-5 with an RGB+VL+stereo ensemble~\cite{schoonbeek2024industreal}. For IKEA ASM activity recognition, I3D-based methods remain the standard baselines (see Table~\ref{tab:ikea-headline}). \todo{Fill in POPW activity recognition results when training completes.}
...
The key advantage of FiLM is that it enables conditioning without requiring gradient flow through the conditioning network, preventing feedback loops while still allowing semantic information to modulate features. This makes FiLM suitable for cross-task information flow where pose estimates influence activity features without backpropagating activity gradients into the pose network.
...
\begin{itemize}
\item \textbf{Input}: multi-scale FPN features (P3+P4+P5) $\rightarrow$ GAP $\rightarrow$ concat $\rightarrow$ MLP($768 \rightarrow 512 \rightarrow 256$) $\rightarrow$ per-frame feature $[B, 256]$
...
VideoMAE V2 features are projected (384→512) and concatenated with the CNN feature vector prior to classification \todo.
...
Following Kendall et al.~(2018)~\cite{kendall2018multi}, the four task losses are weighted:
\begin{equation}
...
Three training stages are defined with progressively expanding active task sets $\mathcal{T}_s$:
\begin{equation}
...
% ANNOTATION [SYNC 2026-07-02]: The staged curriculum above describes the
% DESIGN-SPACE variant. The configuration that produces the reported results
% is NON-STAGED (STAGED_TRAINING=False): all four heads train jointly from
% epoch 0, activity ramps in over ACT_RAMP_EPOCHS=3, EMA (0.995) runs from
% epoch 0, and LDAM-DRW is disabled (plain CE + label smoothing 0.1). Before
% submission, either (a) rewrite this subsection to the non-staged procedure,
% or (b) keep the staged description explicitly as an ablated alternative.
% Reviewers WILL diff this section against the public code.
...
\For{epoch $e = 1, \ldots, E$}
    ...
    \For{each mini-batch $(\mathbf{I}, \mathbf{y})$ in $\mathcal{D}_\text{train}$}
        ...
        \State Accumulate gradients (4 steps)
        \State Clip $\|\nabla\|_2 \leq 5.0$; step optimizer; update EMA (decay$=$0.995)
    \EndFor
...
Evaluation is performed on two assembly video datasets: one third-person multi-view dataset (IKEA ASM) and one egocentric dataset (IndustReal).
...
% [SYNC 2026-07-02] Table synced with the code that actually trains the model
% (src/config.py stage_rf4 preset + src/training/train.py, post-F1..F16 fixes).
% The previous values (Lion, batch 1x32, clip 1.0, EMA 0.999@16, cosine
% restarts) described an older design that was never the shipped configuration.
Batch size & 6 (physical) $\times$ 4 (gradient accumulation) \\
Effective batch & 24 \\
Optimizer & AdamW ($\beta_1{=}0.9, \beta_2{=}0.999$) \\
Backbone LR & $5 \times 10^{-5}$ (0.1$\times$ head LR) \\
Head LR & $5 \times 10^{-4}$ (OneCycle peak $3.75 \times 10^{-4}$) \\
Bias LR & $1.5 \times 10^{-4}$ (0.3$\times$ head LR) \\
Weight decay & $1 \times 10^{-3}$ (0 for bias/norm/loss log-variances) \\
Scheduler & Warmup (2 ep, linear from 0.1$\times$) $\rightarrow$ OneCycleLR (pct$_\text{start}{=}0.1$, cosine anneal, stepped per epoch) \\
Gradient clipping & $\ell_2$-norm $= 5.0$ (global, all heads) \\
Total epochs & 100 \\
EMA decay & 0.995 (active from epoch 0) \\
Seed & 42 (cuDNN benchmark mode; not bitwise-deterministic) \\
\bottomrule
\end{tabular}
\end{table}

\noindent\textbf{Optimizer choice.} AdamW with differential learning rates. The backbone receives $0.1\times$ the head learning rate to preserve ImageNet pretraining; bias parameters receive $0.3\times$ to prevent EMA-locked collapse observed in preliminary experiments. Weight decay is excluded for bias, normalization, and the Kendall log-variance parameters (decaying log-variances would bias all task precisions toward uniform).

\noindent\textbf{Gradient accumulation.} With physical batch size 6 on a 16\,GB GPU (RGB-only configuration, VideoMAE disabled), gradients are accumulated over 4 steps for an effective batch of 24. The OneCycleLR peak learning rate is scaled so the per-sample update intensity matches the $5\times10^{-4}/32$ reference of the single-task baselines (linear scaling rule~\cite{goyal2017accurate}).
...
To properly engage the Causal Transformer in the PSR head, we alternate standard single-frame batches with sequence batches (every 4th step). Each sequence batch consists of $T{=}8$ contiguous frames from the same recording, processed as a $[B, T, C, H, W]$ tensor. The Causal Transformer processes the full temporal sequence with upper-triangular masking. During sequence batches, only the PSR loss is computed (detection, activity, and pose losses are disabled); because the PSR branch consumes detached FPN features, sequence batches leave the shared backbone gradient accumulation untouched.

\subsection{Reproducibility}

All random seeds (Python, NumPy, PyTorch, CUDA) are fixed at 42. cuDNN deterministic mode is enabled. Training logs, configuration files, and evaluation scripts will be released upon acceptance. Results are reported from single runs; standard deviations across 3 seeds are provided where noted (denoted $\pm$).
...
\midrule
ASD mAP & Schoonbeek~2024~\tabulatecite{schoonbeek2024industreal} Table~3 & mAP (b-boxed) & Evaluated on \textit{annotated frames only}, not full videos. ``mAP (entire videos) = 0.641'' is a different protocol. The implementation uses COCO 101-point all-point interpolation (mAP@[0.5:0.95]) as the primary metric. \\
...
PSR F1 & PSRT paper~\tabulatecite{schoonbeek2024industreal} & $\pm3$-frame tolerance, full test set & POPW uses $\pm3$/$\pm5$-frame tolerance bi-directional greedy matching per PSRT paper definition. Best PSRT baseline (B2) achieves F1=0.731 on full IndustReal test set; STORM-PSR achieves F1=0.506. The PSRT supplementary reports B3 F1=0.883 on error-free recordings. \textbf{Note on paradigm comparability:} POPW predicts per-frame component state, while PSRT baselines (B2/B3) and STORM-PSR predict transitions using detection-backbone outputs and procedural/temporal priors. The metric (F1@$\pm T$ frames) is the same, but the prediction paradigm is fundamentally different. Direct numerical comparison requires the same prediction paradigm; POPW's 0.7018 is reported as a per-frame component-state F1, not as a head-to-head transition F1. \\
...
\vspace{0.5em}
\noindent\textbf{Detection mAP reporting convention.} The YOLOv8m 83.80\% result from Schoonbeek~2024 Table~3 is ``mAP (b-boxed),'' meaning \textit{frames with ground-truth bounding boxes only}. Two POPW detection numbers are reported: (1) ``mAP (b-boxed)'' under the same protocol for direct comparison, and (2) ``mAP@0.5 (all frames)'' as an additional reference \todo.

\vspace{0.5em}
\noindent\textbf{Activity evaluation convention.} The MViTv2 baseline numbers are \textit{clip-level} Top-1/Top-5, where each action clip produces a single prediction from 16 uniformly-sampled frames. RGB-only: 65.25\%/87.93\%; RGB+VL+stereo: 66.45\%/88.43\%. The headline activity metric follows this clip-level convention and is NOT directly comparable to STORM-PSR (POPW predicts per-frame component state, STORM-PSR uses procedurally-generated transition features; paradigm difference documented in 150/155/165). Frame-level Top-1 is additionally reported as a secondary metric. See Section~\ref{sec:activity-modality} for modality considerations when comparing RGB-only vs.\ multi-modal baselines.

\label{sec:activity-modality}
\vspace{0.5em}
\noindent\textbf{Activity modality note.} The IndustReal MViTv2 baseline uses RGB+VL (vision-language) + stereo depth as input modalities. POPW uses RGB only. Comparing RGB-only Top-1 against the multi-modal MViTv2 is not fully fair; the VL and stereo modalities provide additional spatial/depth cues that the RGB-only POPW cannot leverage. This modality gap is noted explicitly when comparing against MViTv2.
...
\midrule
\ikea{} (front, RGB)    & P3D~\cite{benshabat2021ikea}                 & 60.40 & 83.13 & --- \\
\ikea{} (front, RGB+pose) & I3D~\cite{benshabat2021ikea}               & 64.15 & 76.55 & --- \\
\ikea{} (all views)     & I3D~\cite{benshabat2021ikea}                 & 47.00 & 80.54 & --- \\
\ikea{} (all views)     & PC3D~\cite{aganian2023ikea}                  & 80.20 & 91.80 & --- \\
\ikea{} (all views)     & \popw{} (ours)                               & \popwres & \popwres & --- \\
\midrule
\industreal{} (RGB-only) & MViTv2 (K400 pretrain)~\cite{schoonbeek2024industreal}       & 65.25 & 87.93 & --- \\
\industreal{} (RGB+VL+stereo) & MViTv2 (K400 pretrain)~\cite{schoonbeek2024industreal}       & 66.45 & 88.43 & --- \\
...
Each task head's contribution is isolated by training single-task variants with the same backbone and FPN. ``All heads'' is the full \popw{} model. Table~\ref{tab:abl-heads} shows results.
...
The activity head's temporal components (Feature Bank, TCN, ViT blocks, VideoMAE V2) are ablated to quantify each contribution.
...
\item Stage transitions: detection mAP typically dips when new tasks activate (gradient competition), then recovers within \todo epochs.
\item Kendall auto-balancing: the learned precisions converge to a ratio reflecting each task's noise level, with detection receiving \todo and activity receiving \todo (highest loss variance).
\item LDAM-DRW transition at epoch \todo: expect a \todo jump in activity macro-F1 as class-balanced weights activate for tail classes.
\end{enumerate}
...
\noindent\textbf{Comparison with separate baselines.} Running YOLOv8m (25.9M, 79.3 GFLOPs) + MViTv2 (34.5M, 64 GFLOPs) + STORM-PSR ($\sim$15M, est.) separately requires $\sim$75.4M parameters and $\sim$143+ GFLOPs for three forward passes. \popw{} (53.3M without VideoMAE) performs all five tasks in a single pass, representing a \todo parameter reduction and an estimated \todo FLOP reduction vs.\ the multi-model pipeline. With VideoMAE V2 enabled (75.3M), the parameter count is comparable, but inference remains a single pass.
...
\begin{itemize}[nosep,leftmargin=*]
\item Parameter efficiency: POPW shares a single ConvNeXt-Tiny backbone for all five tasks vs.\ separate backbones in multi-model baseline
\item FLOPs efficiency: single forward pass vs.\ three separate backbones (YOLOv8m + MViTv2 + STORM-PSR)
\item Representation constraints: shared features must capture task-agnostic patterns useful for all heads
\end{itemize}

\subsection{Which Tasks Benefit Most from Shared Representation?}

The FiLM conditioning mechanism enables cross-task information flow~\todo:
\begin{itemize}
    \item PoseFiLM: body keypoint confidence modulates high-level features ($\gamma, \beta$)~\todo
    \item HeadPoseFiLM: 9-DoF gaze modulates activity features before ActivityHead~\todo
    \item Detection context provides object state information to activity recognition~\todo
    \item Head pose conditioning captures gaze direction~\todo
    \item PSR head: independent multi-scale FPN path avoids interference from other tasks~\todo
\end{itemize}

\subsection{Quantitative Cross-Method Comparison}
\label{sec:quant-comparison}

Efficiency-normalized accuracy comparison across all tasks~\todo:
\begin{itemize}
    \item Detect where POPW's multi-task design provides greatest benefit~\todo
    \item Compare accuracy-per-parameter across tasks~\todo
\end{itemize}

\textbf{Detection (ASD):} YOLOv8m achieves 83.80\% mAP@0.5 with 25.9M parameters and 79.3 GFLOPs on annotated frames. POPW's shared backbone (without VideoMAE) achieves \todo{} mAP@0.5 on the same protocol, representing \todo vs.\ YOLOv8m. Per-task parameter reduction vs.\ running YOLOv8m separately: \todo.

\textbf{Activity recognition:} MViTv2 achieves 65.25\% Top-1 on IndustReal (RGB-only) with 34.5M params and 64 GFLOPs. POPW achieves \todo Top-1 on the same protocol, a gap of \todo compared to MViTv2. Comparison is not fully fair since MViTv2 uses RGB+VL+stereo; see modality discussion in Section~\ref{sec:activity-modality}.

\textbf{Procedure step recognition:} STORM-PSR achieves F1=0.506 and POS=0.812 on IndustReal. POPW achieves \todo F1 (\todo-frame tolerance) and \todo POS on the same dataset, \todo vs.\ STORM-PSR.

\textbf{Head pose:} No published supervised baseline exists for 9-DoF head pose estimation in industrial assembly contexts. POPW's head pose head therefore represents a new capability. Forward angular MAE: \todo$^\circ$. Position MAE: \todo mm.

Overall efficiency-normalized comparison~\todo:
\begin{itemize}
    \item Detection: POPW may provide largest accuracy-per-parameter benefit~\todo
    \item Activity: moderate benefit expected~\todo
    \item PSR: moderate benefit expected~\todo
\end{itemize}
This pattern guides the ablation analysis in Section~\ref{sec:ablation}. Results pending full training completion.

\subsection{Failure Cases}

Multi-task models exhibit failure modes distinct from single-task models. Three primary failure categories observed in validation experiments:
\begin{itemize}
    \item Heavily occluded assemblies
    \item Rare action classes
    \item Cross-view generalization on IKEA ASM
\end{itemize}

\textbf{Heavily occluded assemblies:}~\todo
\begin{itemize}
    \item Hands occlude workspace during tightening/alignment actions
    \item Detection head confidence drops below pose conditioning threshold
    \item PoseFiLM receives near-zero confidence → cross-task flow disabled
    \item Activity head falls back to body-pose-independent features
    \item Estimated Top-1 accuracy degradation on occlusion-heavy sequences: \todo
\end{itemize}

\textbf{Rare action classes:}~\todo
\begin{itemize}
    \item 74 activity classes in IndustReal include transitions (<15 training examples): ``adjust bracket,'' ``flip panel''
    \item LDAM-DRW partially addresses class imbalance
    \item Rare classes may show lower recall than frequent classes (e.g., ``align,'' ``insert'')
    \item Expected recall gap between rare and frequent classes: \todo
    \item Amplification effect in multi-task setting vs.\ single-task: \todo
\end{itemize}

\textbf{Cross-view generalization on IKEA ASM:}~\todo
\begin{itemize}
    \item Front-view-only training may degrade top-view/side-view sequences
    \item Expected cross-view Top-1 degradation (front-only training): \todo
    \item Comparison vs.\ single-task methods (e.g., I3D+pose): \todo
    \item Mitigation via multi-view training data: \todo
\end{itemize}

PSR head temporal misalignment~\todo:
\begin{itemize}
    \item Feature bank slides across action boundaries
    \item Stale features from previous step persist 1--2 frames
    \item PSR predictions lag behind true procedure state
\end{itemize}

\FloatBarrier

% =====================================================================
% SECTION: CONCLUSION — OMITTED FOR PROFESSOR REVIEW
% Results pending full training run completion.
% =====================================================================
% Conclusion section to be written after model training and evaluation.
% Expected content:
%   • POPW summary: unified 5-task architecture, shared backbone
%   • Key contributions: FiLM conditioning, Kendall weighting, staged training
%   • Results summary: comparison vs dedicated baselines on IndustReal
%   • Broader impact: industrial assembly monitoring, privacy considerations
%   • Limitations: edge deployment, head pose specificity, long-horizon reasoning
%   • Future work: adaptive feature bank, VideoMAE integration, KD from teachers
% =====================================================================
...
\href{https://arxiv.org/abs/2007.00394}{arXiv:2007.00394}.
...
\href{https://arxiv.org/abs/2306.14536}{arXiv:2306.14536}.
...
\href{https://arxiv.org/abs/2306.05844}{arXiv:2306.05844}.
...
\href{https://arxiv.org/abs/2508.17025}{arXiv:2508.17025}.
...
in \emph{Proc.\ IEEE/CVF Int.\ Conf.\ on Computer Vision (ICCV)}, 2023.
\href{https://github.com/jbistanbul/MiniROAD}{GitHub: jbistanbul/MiniROAD}.
...
\href{https://doi.org/10.3390/s26082454}{doi: 10.3390/s26082454}.
...
\href{https://arxiv.org/abs/2202.07925}{arXiv:2202.07925}.
\href{https://github.com/happyharrycn/actionformer_release}{Code: actionformer_release}.
...
\href{https://arxiv.org/abs/2310.17323}{arXiv:2310.17323}.
...
\href{https://arxiv.org/abs/2510.12385}{arXiv:2510.12385}.
...
\href{https://arxiv.org/abs/2408.11700}{arXiv:2408.11700}.
...
\href{https://arxiv.org/abs/2402.06121}{arXiv:2402.06121}.
...
\href{https://openaccess.thecvf.com/content_cvpr_2018/papers/Kendall_Multi-Task_Learning_Using_CVPR_2018_paper.pdf}{CVPR 2018 openaccess}.
...
\href{https://arxiv.org/abs/2302.06675}{arXiv:2302.06675}.

% Linear LR scaling rule
\bibitem{goyal2017accurate}
P.~Goyal, P.~Doll\'ar, R.~Girshick, P.~Noordhuis, L.~Wesolowski, A.~Kyrola, A.~Tulloch, Y.~Jia, and K.~He,
``Accurate, Large Minibatch SGD: Training ImageNet in 1 Hour,''
arXiv preprint, 2017.
\href{https://arxiv.org/abs/1706.02677}{arXiv:1706.02677}.
...
\href{https://arxiv.org/abs/1906.07413}{arXiv:1906.07413}.
...
\href{https://openaccess.thecvf.com/content_cvpr_2018/papers/Feng_Wing_Loss_for_CVPR_2018_paper.pdf}{CVPR 2018 openaccess}.
...
\href{https://openaccess.thecvf.com/content/CVPR2022/papers/Liu_A_ConvNet_for_the_2020s_CVPR_2022_paper.pdf}{CVPR 2022 openaccess}.
...
\href{https://github.com/facebookresearch/ConvNeXt-V2}{GitHub: facebookresearch/ConvNeXt-V2}.
...
\href{https://openaccess.thecvf.com/content_ICCV_2017/papers/Lin_Focal_Loss_for_ICCV_2017_paper.pdf}{ICCV 2017 openaccess}.

% FiLM
\bibitem{dumoulin2016learned}
V.~Dumoulin, E.~Perez, N.~Schlaug, et al.,
``Feature-wise Linear Modulation,''
\href{https://arxiv.org/abs/1610.04402}{arXiv:1610.04402}, 2016.
...
\href{https://arxiv.org/abs/1709.07871}{arXiv:1709.07871}.
...
in \emph{Proc.\ IEEE/CVF Conf.\ on Computer Vision and Pattern Recognition (CVPR)}, 2023.
\href{https://arxiv.org/abs/2209.09484}{arXiv:2209.09484}.
...
\href{https://arxiv.org/abs/2209.08691}{arXiv:2209.08691}.

\bibitem{wang2023ego}
H.~Wang, M.~K.~Singh, and L.~Torresani,
``Ego-Only: Egocentric Action Detection without Exocentric Transferring,''
in \emph{Proc.\ IEEE/CVF Conf.\ on Computer Vision and Pattern Recognition (CVPR)}, 2023.
\href{https://arxiv.org/abs/2301.01380}{arXiv:2301.01380}.
...
\href{https://arxiv.org/abs/2011.11730}{arXiv:2011.11730}.
...
\href{https://arxiv.org/abs/2512.11189}{arXiv:2512.11189}.
...
\href{https://arxiv.org/abs/2509.18802}{arXiv:2509.18802} (revised March 2026).

\bibitem{wang2023videomae}
Y.~Wang, K.~Li, et al.,
``VideoMAE V2: Scaling Video Masked Autoencoders for Efficient Video Understanding,''
in \emph{Proc.\ IEEE/CVF Conf.\ on Computer Vision and Pattern Recognition (CVPR)}, 2023.
\href{https://github.com/OpenGVLab/VideoMAEv2}{GitHub: OpenGVLab/VideoMAEv2}.

\bibitem{wu2025tempr1}
T.~Wu, L.~Yang, G.~Zhan, et al.,
``TempR1: Improving Temporal Understanding of MLLMs via Temporal-Aware Multi-Task Reinforcement Learning,''
\href{https://arxiv.org/abs/2512.03963}{arXiv:2512.03963}, 2025.
