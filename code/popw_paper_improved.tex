...
\vspace{0.5em}
\noindent\textbf{Activity evaluation convention.} The MViTv2 baseline numbers are \textit{clip-level} Top-1/Top-5, where each action clip produces a single prediction from 16 uniformly-sampled frames. RGB-only: 65.25\%/87.93\%; RGB+VL+stereo: 66.45\%/88.43\%. The headline activity metric follows this clip-level convention and is directly comparable to the PSRT paper protocol. Frame-level Top-1 is additionally reported as a secondary metric. See Section~\ref{sec:activity-modality} for modality considerations when comparing RGB-only vs.\ multi-modal baselines.
