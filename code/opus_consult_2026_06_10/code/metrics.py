...
def _heatmaps_to_detection(
    ...
    for b in range(B):
        ...
        # GT boxes: smoke-test placeholder path. The real eval (evaluate_all) uses
        # cls_preds + reg_preds from the model and a separately-loaded GT. When this
        # fallback fires, GT is unavailable, so mAP will be 0 — warn so users know.
...
def compute_metrics(
    ...
    mae_raw = results.get('MAE', 0.0)
    mae_component = max(0.0, 1.0 - mae_raw / 10.0) if (isinstance(mae_raw, float) and math.isfinite(mae_raw)) else 0.0
    combined = (
        results['mAP50'] * 0.25 +
